...
(function ($)
{
    $.fn.JsRenderData = function (args)
    {
        ...
        if (sourceUrl != null)
        {
            ...
            $.ajax({
                ...
                {
                    fnRenderData(me, data, isPage, pageSize, pageIndex, sourceUrl, templateId, funCallback);
                    if (funCallback != null && funCallback != undefined)
                        funCallback(data);
                    fnTableBindSort(me, data, isPage, pageSize, pageIndex, sourceUrl, templateId, funCallback, jsRenderASC);
                    //奇偶行效果
        ...
        } else
        {
            if (dataSource != null && dataSource != undefined)
            {
                ...
                if (isPage)
                {
                    me.JsRenderPaging(me, dataSource, isPage, pageSize, pageIndex, sourceUrl, templateId, funCallback);
                }
                if (funCallback != null && funCallback != undefined)
                    funCallback(data);
                fnTableBindSort(me, data, isPage, pageSize, pageIndex, sourceUrl, templateId, funCallback, jsRenderASC);
    ...
    function fnRenderData(me, dataSource, isPage, pageSize, pageIndex, sourceUrl, templateId, funCallback)
    {
        if ($(me).parent().find("tbody").length == 0)
        {
    ...
    $.fn.PageHtml = function (recordCount, pageCount, pageSize, pageIndex)
    {
        ...
        } else
        {
            ...
            //html += "<a index='next' class='next status-default' >&nbsp;</a>";
            //html += "<a index='last' class='last status-default'>末页</a>";
        }
    ...
    //生成分页部分
    $.fn.JsRenderPaging = function (obj, dataS, isPage, pageSize, pageIndex, sourceUrl, dataTemplateId, funCallback)
    {
        ...
        //翻页需要存储数据到table的属性中
        this.setListPageLink(sourceUrl, funCallback, isPage, pageSize, dataTemplateId); //设置分页的链接

        this.setListGotoPageLink(sourceUrl, funCallback, isPage, pageSize, dataTemplateId, pageCount); //设置'转向'分页的事件

        return this;
    };
    //设置分页连接
    $.fn.setListPageLink = function (sourceUrl, funCallback, isPage, pageSize, templateId)
    {
        var me = this;
        $(me).parent().find("#listPage a").unbind("click").click(function ()
        {
            ...
            me.JsRenderData({ sourceUrl: sourceUrl, pageIndex: thisPageIndex, pageSize: pageSize, isPage: isPage, templateID: templateId, funCallback: funCallback });
        });
        return me;
    };

    //设置转向连接
    $.fn.setListGotoPageLink = function (sourceUrl, funCallback, isPage, pageSize, templateId, pageCount)
    {
        var me = this;
        $(me).parent().find("#listPage #btnpagegoto").unbind("click").click(function ()
        {
            ...
            if (o.val() != "")
            {
                ...
                if (pattern.test(o.val()))
                {
                    ...
                    {
                        me.JsRenderData({ sourceUrl: sourceUrl, pageIndex: thisGotoIndex, pageSize: pageSize, isPage: isPage, templateID: templateId, funCallback: funCallback });
                    }
                }
            }
        });
