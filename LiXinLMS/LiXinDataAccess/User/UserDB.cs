            ...
            using (IDbConnection conn = OpenConnection())
            {
                const string sqlwhere =
                    @"update Sys_User set 
                        ...
                        LoginId=@LoginId,DeptPath=@DeptPath,IsMain=@IsMain
                    where UserId=@UserId
";
                var param = new
                {
...
WHERE UserId in ({0})", userID);
        ...
        /// <returns></returns>
        public List<Cl_CourseOrder> GetTogetherList(int uid)
        {
            using (IDbConnection conn = OpenConnection())
            {
                string sqlwhere = string.Format(@"SELECT temp.* FROM (
SELECT cco.*,cc.IsMust,cc.Way ,case WHEN (dbo.F_IsAppStatus(cc.Id,cco.UserId)=1)  THEN cco.GetScore  ELSE  0 END AS tmpScore 
FROM Cl_CourseOrder AS cco LEFT JOIN Co_Course AS cc ON cc.Id=cco.CourseId WHERE cco.UserId=@UserId AND cc.Way=1) AS temp 
WHERE  temp.tmpScore>0");
                return conn.Query<Cl_CourseOrder>(sqlwhere, new { UserId = uid }).ToList();
            }
        }
        /// <summary>
        /// 视频授课
        ...
        /// <param name="uid"></param>
        /// <returns></returns>
        public List<Cl_CourseOrder> GetVideoList(int uid)
        {
            using (IDbConnection conn = OpenConnection())
            {
                string sqlwhere = string.Format(@"SELECT temp.* FROM (
...
LEFT JOIN Co_Course AS cc ON cc.Id=ccs.CourseId WHERE ccs.UserId=@UserId AND ccs.CpaFlag =0) AS temp 
WHERE  temp.GetLength>0");
                return conn.Query<Cl_CourseOrder>(sqlwhere, new { UserId = uid }).ToList();
            }
        }
...
(SELECT sum(GetLength) FROM Cl_CpaLearnStatus WHERE UserID=@UserId AND GetLength>=0 and CpaFlag!=0 ) as CPAScore,(SELECT count(*) FROM View_OrderTimes WHERE UserId=@UserId) AS ordertimes FROM Sys_User AS su LEFT JOIN Sys_Department AS sd ON su.DeptId=sd.DepartmentId WHERE su.UserId=@UserId");
                return conn.Query<Sys_User>(sqlwhere, new
                {
                    UserId = uid
                }).FirstOrDefault();
...
(dbo.F_IsAppStatus(Cl_CourseOrder.Id,UserId)=1)
Group by UserId 
union ALL 
...
(SELECT row_number()OVER(ORDER BY Id {1}) numb,count(*)OVER(PARTITION BY null) totalcount,* FROM(
	...
	CASE WHEN (dbo.F_IsAppStatus(cc.Id,@UserId)=1) then clc.GetScore  ELSE 0 END GetScore,
	...
	WHERE cc.YearPlan=@YearPlan AND cc.IsDelete=0 AND cc.CourseFrom=2 AND clc.OrderStatus!=0  AND clc.UserId=@UserId and {0} and cc.Way=1
 ) result WHERE  GetScore>0) a ) t
WHERE numb BETWEEN @startLength*(@startIndex-1)+1  AND @startLength*@startIndex", where, Order);
        ...
        public List<Co_CourseShow> GetMyVidioCourse(int userid, int year, int startIndex = 1, int startLength = int.MaxValue, string Order = "asc", string where = "1=1")
        {
            using (var conn = OpenConnection())
            {
                string sql = @"SELECT * FROM (SELECT row_number()OVER(ORDER BY cc.Id " + Order + ") numb,count(*)OVER(PARTITION BY null) totalcount,sum(clc.GetLength)OVER(PARTITION BY null) AS passsum,cc.Id,cc.CourseName,CourseLength,Way,StartTime,EndTime,cc.IsMust,sc.RoomName,clc.GetLength as GetScore,isnull(ccp.Id,0) AS CoPaperID,isnull(ccp.PaperId,0) AS PaperId FROM Cl_CpaLearnStatus AS clc LEFT JOIN Co_Course AS cc ON cc.Id=clc.CourseId LEFT JOIN Sys_ClassRoom sc ON sc.Id=cc.RoomId LEFT JOIN Co_CoursePaper AS ccp ON (ccp.CourseId=clc.CourseId and ccp.IsMain=0) WHERE cc.IsDelete=0 AND cc.CourseFrom=2 AND clc.CpaFlag=0 AND clc.IsPass=1 AND clc.GetLength>=0 and cc.YearPlan=@YearPlan and clc.UserId=@UserId and " + where + ") result WHERE numb BETWEEN @startLength*(@startIndex-1)+1  AND @startLength*@startIndex";
                var param = new
                {
                    startIndex = startIndex,
                    startLength = startLength,
        ...
        public List<Co_CourseShow> GetMyCPACoursebyway(int userid, int year, int startIndex = 1, int startLength = int.MaxValue, string Order = "asc", string where = "1=1")
        {
            using (IDbConnection conn = OpenConnection())
            {
                string sql = @"SELECT * FROM (SELECT row_number()OVER(ORDER BY cc.Id " + Order + ") numb,count(*)OVER(PARTITION BY null) totalcount,sum(clc.GetLength)OVER(PARTITION BY null) AS passsum,cc.Id,cc.CourseName,CourseLength,Way,StartTime,EndTime,CourseAddress,clc.GetLength as GetScore,clc.CpaFlag FROM Cl_CpaLearnStatus AS clc LEFT JOIN Co_Course AS cc ON cc.Id=clc.CourseId WHERE cc.YearPlan=@YearPlan AND cc.IsDelete=0 AND cc.CourseFrom=2 and clc.CpaFlag=1 and clc.GradeStatus=1 AND clc.GetLength>=0 and clc.UserID=@UserID and " + where + ") result WHERE numb BETWEEN @startLength*(@startIndex-1)+1  AND @startLength*@startIndex";
                var param = new
                {
                    startIndex = startIndex,
                    startLength = startLength,
        ...
        /// <param name="where"></param>
        /// <returns></returns>
        public List<Co_CourseShow> GetMyCPACourse(int userid, int year, int startIndex = 1, int startLength = int.MaxValue, string Order = "asc", string where = "1=1")
        {
            using (IDbConnection conn = OpenConnection())
            {
                string sql = @"SELECT * FROM (SELECT row_number()OVER(ORDER BY cc.Id " + Order + ") numb,count(*)OVER(PARTITION BY null) totalcount,sum(clc.GetLength)OVER(PARTITION BY null) AS passsum,isnull((SELECT sum(clc.GetLength) FROM Cl_CpaLearnStatus AS clc LEFT JOIN Co_Course AS cc ON cc.Id=clc.CourseId WHERE cc.YearPlan=@YearPlan AND cc.IsDelete=0 AND cc.CourseFrom=2 and clc.UserID=@UserID AND cc.Way=3 AND cc.IsMust=1 and clc.CpaFlag>0 and clc.GradeStatus=1 ),0) AS daode,cc.Id,cc.CourseName,CourseLength,Way,StartTime,EndTime,clc.GetLength,clc.CpaFlag FROM Cl_CpaLearnStatus AS clc LEFT JOIN Co_Course AS cc ON cc.Id=clc.CourseId WHERE cc.YearPlan=@YearPlan AND cc.IsDelete=0 AND cc.CourseFrom=2 and clc.CpaFlag>0 and clc.GradeStatus=1 AND clc.GetLength>=0 and clc.UserID=@UserID and " + where + ") result WHERE numb BETWEEN @startLength*(@startIndex-1)+1  AND @startLength*@startIndex";
//                string sql = string.Format(@"SELECT * FROM(
//SELECT *,sum(GetLength)OVER(PARTITION BY null) AS passsum FROM (
//SELECT row_number()OVER(ORDER BY cc.Id {0}) numb,count(*)OVER(PARTITION BY null) totalcount,
//--sum(clc.GetLength)OVER(PARTITION BY null) AS passsum
//isnull((SELECT sum(clc.GetLength) 
//FROM Cl_CpaLearnStatus AS clc LEFT JOIN Co_Course AS cc ON cc.Id=clc.CourseId 
//WHERE cc.YearPlan=@YearPlan AND cc.IsDelete=0 AND cc.CourseFrom=2 and clc.UserID=@UserID   AND cc.Way=3 AND cc.IsMust=1 and clc.CpaFlag>0 and clc.GradeStatus=1 ),0) AS daode,
//
//cc.Id,cc.CourseName,CourseLength,Way,StartTime,EndTime,
//CASE WHEN  (dbo.F_IsAppStatus(cc.Id,@UserId)=1)  THEN  clc.GetLength ELSE 0 END GetLength
//,clc.CpaFlag 
//FROM Cl_CpaLearnStatus 
//AS clc LEFT JOIN Co_Course AS cc ON cc.Id=clc.CourseId WHERE cc.YearPlan=@YearPlan  AND cc.IsDelete=0 AND cc.CourseFrom=2 and clc.CpaFlag>0 and clc.GradeStatus=1 AND clc.GetLength>=0 and clc.UserID=@UserID   and {1}) result 
//) a WHERE numb BETWEEN @startLength*(@startIndex-1)+1  AND @startLength*@startIndex
//", Order, where);
                var param = new
                {
                    startIndex = startIndex,
        ...
        /// <param name="where"></param>
        /// <returns></returns>
        public List<Co_CourseShow> GetMyCPACourse(int userid, string bentime, string endtime, int startIndex = 1, int startLength = int.MaxValue, string Order = "asc", string where = "1=1")
        {
            using (IDbConnection conn = OpenConnection())
            {
                string sql = @"SELECT * FROM (SELECT row_number()OVER(ORDER BY cc.Id " + Order + ") numb,count(*)OVER(PARTITION BY null) totalcount,sum(clc.GetLength)OVER(PARTITION BY null) AS passsum,isnull((SELECT sum(clc.GetLength) FROM Cl_CpaLearnStatus AS clc LEFT JOIN Co_Course AS cc ON cc.Id=clc.CourseId WHERE ( cc.StartTime BETWEEN '" + bentime + "' AND '" + endtime + "' ) AND cc.IsDelete=0 AND cc.CourseFrom=2 AND cc.Way=3 and clc.UserID=@UserID AND cc.IsMust=1 and clc.CpaFlag>0 and clc.GradeStatus=1 ),0) AS daode,cc.Id,cc.CourseName,CourseLength,Way,StartTime,EndTime,clc.GetLength,clc.CpaFlag FROM Cl_CpaLearnStatus AS clc LEFT JOIN Co_Course AS cc ON cc.Id=clc.CourseId WHERE ( cc.StartTime BETWEEN '" + bentime + "' AND '" + endtime + "' ) AND cc.IsDelete=0 AND cc.CourseFrom=2 and clc.CpaFlag>0 and clc.GradeStatus=1 AND clc.GetLength>=0 and clc.UserID=@UserID and " + where + ") result WHERE  numb BETWEEN @startLength*(@startIndex-1)+1  AND @startLength*@startIndex";
//                string sql = string.Format(@"SELECT * FROM (
//SELECT *,sum(GetLength)OVER(PARTITION BY null) AS passsum FROM (
//    SELECT row_number()OVER(ORDER BY cc.Id {0}) numb,count(*)OVER(PARTITION BY null) totalcount,
//
//    isnull((SELECT sum(clc.GetLength) FROM Cl_CpaLearnStatus AS clc
//     LEFT JOIN Co_Course AS cc ON cc.Id=clc.CourseId 
//     WHERE 
//     ( cc.StartTime BETWEEN '{1}' AND '{2}' ) 
//     AND cc.IsDelete=0 AND cc.CourseFrom=2 AND cc.Way=3 and clc.UserID=@UserID AND cc.IsMust=1 and clc.CpaFlag>0 and clc.GradeStatus=1 ),0) 
//     AS daode,cc.Id,cc.CourseName,
//     CourseLength,Way,StartTime,EndTime,
//     	CASE WHEN  (dbo.F_IsAppStatus(Cl_CourseOrder.Id,Cl_CourseOrder.UserId)=1) 
//	THEN  clc.GetLength ELSE 0 END GetLength
//     ,clc.CpaFlag FROM Cl_CpaLearnStatus AS clc LEFT JOIN Co_Course AS cc ON cc.Id=clc.CourseId WHERE ( cc.StartTime BETWEEN '{1}' AND '{2}' ) AND cc.IsDelete=0 AND cc.CourseFrom=2 and clc.CpaFlag>0 and clc.GradeStatus=1 AND clc.GetLength>=0 and clc.UserID=@UserID and {3}) result 
//) a 
//WHERE  numb BETWEEN @startLength*(@startIndex-1)+1  AND @startLength*@startIndex", Order, bentime, endtime, where);
