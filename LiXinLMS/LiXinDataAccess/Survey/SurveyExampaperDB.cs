...
{
    public partial class SurveyManageDB : BaseRepository
    {
        /// <summary>
        /// 根据分类ID获取问卷
        /// </summary>
        ...
        public Survey_Exampaper GetServeyExamPaper(int paperID)
        {
            ...
            {
                const string sqlstr = "select * from Survey_Exampaper where IsDelete=0 AND ExampaperID=@paperID";
        ...
        /// <returns></returns>
        public List<Survey_Exampaper> GetServeyExamPaperList(string where = " Survey_Exampaper.IsDelete=0 ", string orderBy = " order by Survey_Exampaper.ExampaperID desc ")
        {
            using (var connection = OpenConnection())
            {
                var query = string.Format(@"select Survey_Exampaper.*,Survey_ExampaperCategory.CategoryName as SortName from Survey_Exampaper,Survey_ExampaperCategory where Survey_ExampaperCategory.CategoryId=Survey_Exampaper.SortID and {0} {1} ", where, orderBy);
        ...
        /// <param name="orderBy">排序条件</param>
        /// <returns></returns>
        public List<Survey_Exampaper> GetServeyExamPaperListPaging(out int totalCount, string where = " Survey_Exampaper.IsDelete=0 ", int startIndex = 0, int pageLength = int.MaxValue, string orderBy = " order by Survey_Exampaper.LastUpdateTime desc ")
        {
            using (var connection = OpenConnection())
            {
                totalCount = connection.Query<int>("select count(1) from Survey_Exampaper where " + where).First();
                var query = string.Format(@"
select top {0} * from (
    select  row_number() over(order by {1} ) as rowNum,Survey_Exampaper.*,Survey_ExampaperCategory.CategoryName as SortName from Survey_Exampaper,Survey_ExampaperCategory
    where Survey_ExampaperCategory.CategoryId=Survey_Exampaper.SortID and " + where + @" 
) result 
where rowNum between  @startLength*(@startIndex-1)+1  AND @startLength*@startIndex", pageLength, orderBy);
        ...
        public bool DeleteServeyExamPaper(int id)
        {
            ...
            {
                const string sqlwhere =
                    @"update Survey_Exampaper set IsDelete=1 where ExampaperID=@id";
                var param = new
                {
                    id
                };
        ...
        public bool DeleteServeyExamPaper(string ids)
        {
            ...
            {
                string sqlwhere =
                  string.Format(@"update Survey_Exampaper set IsDelete=1 where ExampaperID in ({0})", ids);
        ...
        public bool IsExsitServeyExamPaper(string paperName, int paperID, int sortID)
        {
            ...
            {
                const string sqlwhere =
                    @"select count(*) from Survey_Exampaper where ExamTitle=@paperName and ExampaperID<>@paperID and SortID=@sortID";
                var param = new
                {
                    paperName,
                    paperID,
        ...
        public bool DeleteByExampaperId(int exampaperId)
        {
            ...
            {
                const string deleteSql = @"UPDATE Survey_Question
SET Status=1
WHERE ExampaperID= @eId";
        ...
        public bool DeleteAnswerById(int exampaperId)
        {
            using (IDbConnection connection = OpenConnection())
            {
                const string deleteSql = @"UPDATE Survey_QuestionAnswer
set  Status=1
WHERE QuestionID in (
SELECT QuestionID FROM Survey_Question
WHERE ExampaperID=@eId)";
                var param = new
                {
                    eId = exampaperId
        ...
        public List<Survey_Exampaper> GetSurveyByID(int paperID)
        {
            using (var conn = OpenConnection())
            {
                const string sql = @"SELECT se.*,sec.CategoryName FROM Survey_Exampaper se
                                     LEFT JOIN Survey_ExampaperCategory sec ON se.SortID=sec.CategoryId
        ...
        public List<Survey_Question> GetQuestionByID(int paperID)
        {
            using (var conn = OpenConnection())
            {
                const string sql = @"SELECT * FROM Survey_Question
        ...
        public List<Survey_QuestionAnswer> GetQuestionAnswerByID(int paperID)
        {
            using (var conn = OpenConnection())
            {
                const string sql = @"SELECT * FROM Survey_QuestionAnswer
                                    WHERE QuestionID  IN (SELECT QuestionID  FROM Survey_Question
                                    WHERE ExampaperID=@paperID
                                    )";
                var param = new
                {
