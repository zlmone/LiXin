    ...
    public class TrainGradeManageController : BaseController
    {
        ...
        public JsonResult GetUserTrain(string realName = "", string deptName = "", string JobNum = "", string payGrade = "", int pageIndex = 1, int pageSize = int.MaxValue, string jsRenderSortField = "")
        {
            try
            {
                ...
                {
                    var user = userBL.GetUserByHrID(CurrentUser.LeaderID);
                    trainUserID = user == null ? trainUserID : user.UserId;
                }
                ...
                var list = trainBL.GetUserTrain(out totalCount, trainUserID, pageIndex, pageSize, realName.ReplaceSql(), JobNum.ReplaceSql(), payGrade.ReplaceSql(), deptName, jsRenderSortField == "" ? "ORDER BY su.DeptPath asc,su.TrainGrade asc" : ("order by su." + jsRenderSortField));
        ...
        /// 是否是培训负责人
        /// </summary>
        public JsonResult UpdateTrainMaster(int userID, int TrainMaster)
        {
            try
            {
                //trainBL.UpdateAllTrainMaster(CurrentUser.UserId);
                trainBL.UpdateTrainMaster(userID, TrainMaster);
