...
namespace LiXinControllers
{
    public partial class CourseManageController : BaseController
    {
        ...
        /// <returns></returns>
        public ActionResult CompetencyCourseList(int showGrade = 0, string CCcourseIds = "", int showCCManage = 0, int showCCCheckbox = 0, int showCCSureButton = 0, int showAddCCButton = 0)
        {
            if (showGrade == 1)
            {
        ...
        /// <returns></returns>
        public JsonResult GetCompetencyCourseList(string sortIds, string gradeIds, string courseName, int IsMust = -1, int pageSize = 20, int pageIndex = 1)
        {
            int totalCount = 0;
            string strWhere = " 1=1 ";
            ...
            List<int> listCourseIds = _sysPayGradeBL.GetSys_SortLinkGradeList(strWhere).Select(i => i.CourseId).ToList();
