...
namespace LiXinControllers.AllTeacherCourse
{
    public class AllTeacherCourseController : BaseController
    {
        ...
        public AllTeacherCourseController(ICourseOrder _CourseOrderBL, ICo_Course _CoCourseBL, ICl_Attendce _ClAttendceBL, ICl_CourseAdvice _ClCourseAdviceBL, ISurveyQuestion _ISurveyQuestionBL, ISurveyQuestionAnswer _ISurveyQuestionAnswerBL, ISurveyReplyAnswer _ISurveyReplyAnswerBL, ICo_CourseResource _ICoCourseResourceBL, IUser _IuserBL, IExamination _IExaminationBL)
        {
        ...
        public ActionResult ALLCourseList(string p)
        {
            ViewBag.trainGrade = AllTrainGrade;
            ...
            if (p != null && p != "" && p == "1")
            {
                if (Session["tcpage"] != null)
                {
                    string sess = Session["tcpage"].ToString();
                    string[] allc = Regex.Split(sess, "㉿", RegexOptions.IgnoreCase);

                    ViewBag.page = allc[0];
                    ViewBag.cname = allc[1] == "" ? "请输入搜索内容" : allc[1];
                    ViewBag.cmust = allc[2];
                    ViewBag.cstate = allc[3];
                    ViewBag.csort = allc[4];
                    ViewBag.cstartTime = allc[5];
                    ViewBag.cendTime = allc[6];
                }
        ...
        public JsonResult GetTeacherOnLineTest(int courseid, int pageSize = 10, int pageIndex = 1)
        {
            ...
            var litme = 0;
            var courseorder = CourseOrderBL.GetTeacherOnLineTest(out litme, courseid, 0,
...
WHERE ID IN (SELECT ID FROM dbo.F_SplitIDs('" + openLevel + "')))>0";
        ...
        public JsonResult FALLTeacherCourseList(string CourseName, int pageSize = 10, int pageIndex = 1, string openLevel = "", int status = -1, int sort = -1,string start = "", string end = "")
        {
            int number = 0;
...
WHERE ID IN (SELECT ID FROM dbo.F_SplitIDs('" + openLevel + "')))>0";
            ...
            if (Session["tcpage"] != null)
            {
                Session.Remove("tcpage");
            }
            Session["tcpage"] = pageIndex + "㉿" + CourseName + "㉿" + openLevel + "㉿" + status + "㉿" + sort + "㉿" + start + "㉿" + end;
            return Json(new
            {
                result = 1,
                dataList = teacherlist,
