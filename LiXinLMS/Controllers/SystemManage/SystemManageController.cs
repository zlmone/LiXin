...
namespace LiXinControllers
{
    public partial class SystemManageController : BaseController
    {
        ...
        public JsonResult GetSysLogList(string logContent, string userName, int logType, string logTimeStart, string logTimeEnd, int pageSize = 20, int pageIndex = 1)
        {
            List<Sys_User> listUser = userBL.GetList();
            List<Sys_Log> listLog = logBL.GetAllList();
            ...
            if (logType != -1)
            {
                listLog = listLog.Where(l => l.LogType == logType).ToList();
            }
            if (!string.IsNullOrEmpty(logContent))
            {
                listLog = listLog.Where(l => l.LogContent.ToLower().Contains(logContent.ToLower())).ToList();
            }
            if (!string.IsNullOrEmpty(logTimeStart))
            {
                listLog = listLog.Where(l => l.LogTime.ToLocalTime() > logTimeStart.StringToDate(0)).ToList();
            }
            if (!string.IsNullOrEmpty(logTimeEnd))
            {
                listLog = listLog.Where(l => l.LogTime.ToLocalTime() < logTimeEnd.StringToDate(1)).ToList();
            }
            if (!string.IsNullOrEmpty(userName))
            {
                listUser = listUser.Where(p => p.Realname.ToLower().Contains(userName.ToLower())).ToList();
                var tempUser = listUser.Select(p => p.UserId);
                listLog = listLog.Where(p => tempUser.Contains(p.CurrentUserId)).ToList();
            }

            int totalCount = listLog.Count();
            listLog = listLog.Skip((pageIndex - 1) * pageSize).Take(pageSize).ToList();

            int n = 0;
            var itemArray = new object[listLog.Count()];
            foreach (var item in listLog)
            {
                ...
                var temp = new
                {
                    ...
                    LogType = typeof(LiXinModels.Enums.LogType).GetEnumName(item.LogType),
                    UserName = user == null ? "该用户已删除" : user.Realname,
            ...
            try
            {
                ...
                foreach (Sys_User u in userlist)
                {
                    if (!string.IsNullOrWhiteSpace(u.Email))
                        ...
                        emailList.Add(new KeyValuePair<string, string>(u.Email, Content));
                    if (!string.IsNullOrWhiteSpace(u.MobileNum))
                        // telephone.Add(u.MobileNum);
                        messlist.Add(new KeyValuePair<string, string>(u.MobileNum, Content));
                }
                if (Must == 1)
                {
                    if (emailList.Count > 0)  
                        //SendEmail(EmailAddress, Title, Content);
                        SendEmail(emailList, Title);
                }
                else if (Must == 2)
                {
                    if (messlist.Count > 0)
