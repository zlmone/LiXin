        ...
        private void SyncUser(List<LiXinModels.User.Sys_Department> deptList, int flag)
        {
            ...
            if (userList.Count > 0)
            {
                ...
                if (flag == 0)
                {
                    ...
                    //hr中的用户变更数据
                    var hrUserChangeList = BaseModel.GetModelsBySql<LiXinSync.User.Sys_UserChangeHR>(string.Format("select tId,colName,tableName,makeDate,modifyType from {0} where makeDate >= '{1}' and tableName = 'A01' ", ConfigurationManager.AppSettings["HRUserChangeTable"], DateTime.Now.Date.AddDays(-2)), da);
                    foreach (var userChange in hrUserChangeList.OrderBy(p=>p.makeDate))
                    {
                        try
                        {
                            ...
                            if (string.IsNullOrWhiteSpace(item.A0101) || string.IsNullOrWhiteSpace(item.A01155))
                                ...
                                model.Status = model.Status == 2 ? 0 : model.Status;
                            ...
                            else
                            {
                                ...
                                switch (item.A01187)
                                {
                                    case "1": traingrade = "A"; break;
                                    case "2": traingrade = "B"; break;
                                    case "3": traingrade = "C"; break;
                                    case "4": traingrade = "D"; break;
                                    case "5": traingrade = "N/A"; break;
                                    default:
                ...
                foreach (var item in hrUserList)
                {
                    try
                    {
                        ...
                        var model = new LiXinModels.User.Sys_User
                        {
                            ...
                            CPA = string.IsNullOrEmpty(item.A01111) ? "否" : (item.A01111 == "是" ? "是" : "否"),
