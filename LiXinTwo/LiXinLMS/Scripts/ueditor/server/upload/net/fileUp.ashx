...
public class fileUp : IHttpHandler, IRequiresSessionState
{
    public void ProcessRequest(HttpContext context)
    {
        ...
        String pathbase = "../uploadfiles/";
        //上传配置
        //TODO:上传文件
        if (context.Session["userID"] != null)
        {
            pathbase += context.Session["userID"] + "/";
        }
        else
        {
            pathbase += "Test" + "/";
        ...
        try
        {
            ...
            if (state == "SUCCESS")
            {
                filename = DateTime.Now.ToString("yyyy-MM-dd-ss") + Guid.NewGuid() + currentType;
        ...
        //向浏览器返回数据json数据
        context.Response.Write("{'url':'" + url + "','title':'" + title + "','state':'" + state + "','fileType':'" +
                               currentType + "'}");

        //context.Response.Write("{'url':'"+ title + "','state':'" + state + "','fileType':'" + currentType + "'}");
    }
