(function() {
    ...
    var utils = UE.utils = {
        ...
        isArray: function(array) { return Object.prototype.toString.apply(array) === "[object Array]"; },
        ...
        bind: function(fn, this_) { return function() { return fn.apply(this_, arguments); }; },
        ...
        unhtml: function(str) { return str ? str.replace(/[&<">]/g, function(m) { return { "<": "&lt;", "&": "&amp", '"': "&quot;", ">": "&gt;" }[m]; }) : ""; },
    ...
    EventBase.prototype = {
        addListener: function(type, listener) { getListener(this, type, true).push(listener); },
    ...
    var domUtils = dom.domUtils = {
        NODE_ELEMENT: 1, NODE_DOCUMENT: 9, NODE_TEXT: 3, NODE_COMMENT: 8, NODE_DOCUMENT_FRAGMENT: 11, POSITION_IDENTICAL: 0, POSITION_DISCONNECTED: 1, POSITION_FOLLOWING: 2, POSITION_PRECEDING: 4, POSITION_IS_CONTAINED: 8, POSITION_CONTAINS: 16, fillChar: ie && browser.version == "6" ? "\ufeff" : "\u200B", keys: { 8: 1, 46: 1, 16: 1, 17: 1, 18: 1, 37: 1, 38: 1, 39: 1, 40: 1, 13: 1 },
        ...
        insertAfter: function(node, nodeToInsert) { return node.parentNode.insertBefore(nodeToInsert, node.nextSibling); },
        ...
        getNextDomNode: function(node, startFromChild, filter, guard) { return getDomNode(node, "firstChild", "nextSibling", startFromChild, filter, guard); },
            ...
            if (k) {
                while (k--) {
                    ...
                    if (obj.addEventListener) {
                        ...
                        if (!handler._d[key]) {
                            handler._d[key] = function(evt) { return handler.call(evt.srcElement, evt || window.event); };
        ...
        unselectable: gecko ? function(node) { node.style.MozUserSelect = "none"; } : webkit ? function(node) { node.style.KhtmlUserSelect = "none"; } : function(node) {
        ...
        removeClasses: function(element, classNames) { element.className = (" " + element.className + " ").replace(new RegExp("(?:\\s+(?:" + classNames.join("|") + "))+\\s+", "g"), " "); },
        ...
        setStyle: function(element, name, value) { element.style[utils.cssStyleToDomStyle(name)] = value; },
        ...
        isFillChar: function(node) { return node.nodeType == 3 && !node.nodeValue.replace(new RegExp(domUtils.fillChar), "").length; },
    ...
    (function() {
        ...
        Range.prototype = {
            ...
            setStart: function(node, offset) { return setEndPoint(true, node, offset, this); },
            setEnd: function(node, offset) { return setEndPoint(false, node, offset, this); },
            setStartAfter: function(node) { return this.setStart(node.parentNode, domUtils.getNodeIndex(node) + 1); },
            setStartBefore: function(node) { return this.setStart(node.parentNode, domUtils.getNodeIndex(node)); },
            setEndAfter: function(node) { return this.setEnd(node.parentNode, domUtils.getNodeIndex(node) + 1); },
            setEndBefore: function(node) { return this.setEnd(node.parentNode, domUtils.getNodeIndex(node)); },
            selectNode: function(node) { return this.setStartBefore(node).setEndAfter(node); },
            selectNodeContents: function(node) { return this.setStart(node, 0).setEnd(node, node.nodeType == 3 ? node.nodeValue.length : node.childNodes.length); },
            cloneRange: function() {
                    ...
                    if (container.nodeType == 3) {
                        if (!offset) {
                            ...
                            if (offset >= container.nodeValue.length) {
                                r["set" + c.replace(/(\w)/, function(a) { return a.toUpperCase(); }) + "After"](container);
                            }
                        }
                    }
                }
            ...
            setCursor: function(toEnd, notFillData) { return this.collapse(toEnd ? false : true).select(notFillData); },
            ...
            applyInlineStyle: function(tagName, attrs, list) {
                ...
                this.trimBoundary().enlarge(false, function(node) { return node.nodeType == 1 && domUtils.isBlockElm(node); }).adjustmentBoundary();
                var bookmark = this.createBookmark(), end = bookmark.end, filterFn = function(node) { return node.nodeType == 1 ? node.tagName.toLowerCase() != "br" : !domUtils.isWhitespace(node); }, current = domUtils.getNextDomNode(bookmark.start, false, filterFn), node, pre, range = this.cloneRange();
                while (current && (domUtils.getPosition(current, end) & domUtils.POSITION_PRECEDING)) {
                    if (current.nodeType == 3 || dtd[tagName][current.tagName]) {
                        ...
                        while (node && (node.nodeType == 3 || dtd[tagName][node.tagName]) && node !== end) {
                            pre = node;
                            node = domUtils.getNextDomNode(node, node.nodeType == 1, null, function(parent) { return dtd[tagName][parent.tagName]; });
                        }
            ...
            removeInlineStyle: function(tagName) {
                ...
                var current = domUtils.getNextDomNode(bookmark.start, false, function(node) { return node.nodeType == 1; }), next;
                while (current && current !== bookmark.end) {
                    next = domUtils.getNextDomNode(current, true, function(node) { return node.nodeType == 1; });
    ...
    (function() {
        ...
        Selection.prototype = {
            ...
            isFocus: function() { return browser.ie && _getIERange(this) || !browser.ie && this.getNative().rangeCount ? true : false; },
            getRange: function() {
                var me = this;
            ...
            getText: function() {
                if (this.isFocus()) {
                    var nativeSel = this.getNative(), nativeRange = browser.ie ? nativeSel.createRange() : nativeSel.getRangeAt(0);
    ...
    (function() {
        ...
        Editor.prototype = {
            ...
            _setup: function(doc) {
                ...
                doc.write((ie && browser.version < 9 ? "" : "<!DOCTYPE html>") + '<html xmlns="http://www.w3.org/1999/xhtml"' + (!useBodyAsViewport ? ' class="view"' : "") + "><head>" + (options.iframeCssUrl ? '<link rel="stylesheet" type="text/css" href="' + utils.unhtml(/^http/.test(options.iframeCssUrl) ? options.iframeCssUrl : (options.UEDITOR_HOME_URL + options.iframeCssUrl)) + '"/>' : "") + '<style type="text/css">' + (options.initialStyle || " ") + "</style></head><body" + (useBodyAsViewport ? ' class="view"' : "") + "></body></html>");
                ...
                if (browser.gecko && browser.version <= 10902) {
                    ...
                    setInterval(function() { me.body.style.height = me.iframe.offsetHeight - 20 + "px"; }, 100);
                }
            },
            ...
            getContent: function(cmd) {
                ...
                var reg = new RegExp(domUtils.fillChar, "g"), html = this.document.body.innerHTML.replace(reg, "").replace(/>[\t\r\n]*?</g, "><");
            ...
            focus: function() { this.selection.getRange().select(true); },
            _initEvents: function() {
                ...
                domUtils.on(source, "dragstart", function() { innerDrag = 1; });
            ...
            _proxyDomEvent: function(evt) { return this.fireEvent(evt.type.replace(/^on/, ""), evt); },
            ...
            _callCmdFn: function(fnName, args) {
                ...
                cmdFn = (cmd = this.commands[cmdName]) && cmd[fnName] || (cmd = UE.commands[cmdName]) && cmd[fnName];
            ...
            queryCommandState: function(cmdName) { return this._callCmdFn("queryCommandState", arguments); },
            queryCommandValue: function(cmdName) { return this._callCmdFn("queryCommandValue", arguments); },
        ...
        return {
            request: function(url, ajaxOptions) {
                var ajaxRequest = creatAjaxRequest(), timeIsOut = false, defaultAjaxOptions = {
                    method: "POST", timeout: 5000, async: true, data: {},
                    onsuccess: function() {
                    },
                    onerror: function() {
                    }
    ...
    UE.commands["inserthtml"] = {
        execCommand: function(command, html) {
            var me = this, range, deletedElms, i, ci, div, tds = me.currentSelectedArr;
    ...
    UE.plugins["autotypeset"] = function() {
        ...
        var me = this, opt = me.options.autotypeset, remainClass = { "selectTdClass": 1, "pagebreak": 1, "anchorclass": 1 }, remainTag = { "li": 1 }, tags = { div: 1, p: 1 }, highlightCont;
                    ...
                    if (html) {
                        ...
                        case "center":
                            if (me.queryCommandValue("imagefloat") != "center") {
                                ...
                                tmpNode = img;
                                while (pN && domUtils.getChildCount(pN, function(node) { return !domUtils.isBr(node) && !domUtils.isWhitespace(node); }) == 1 && (dtd.$inline[pN.tagName] || pN.tagName == "A")) {
                                    tmpNode = pN;
                                    pN = pN.parentNode;
                                }
    ...
    UE.commands["insertimage"] = {
        execCommand: function(cmd, opt) {
            ...
            if (img && /img/i.test(img.tagName) && img.className != "edui-faked-video" && !img.getAttribute("word_img")) {
                ...
                ci = opt[0];
                if (opt.length == 1) {
                    str = '<img src="' + ci.src + '" ' + (ci.data_ue_src ? ' data_ue_src="' + ci.data_ue_src + '" ' : "") + (ci.width ? 'width="' + ci.width + '" ' : "") + (ci.height ? ' height="' + ci.height + '" ' : "") + (ci["floatStyle"] && ci["floatStyle"] != "center" ? ' style="float:' + ci["floatStyle"] + ';"' : "") + (ci.title ? ' title="' + ci.title + '"' : "") + ' border="' + (ci.border || 0) + '" hspace = "' + (ci.hspace || 0) + '" vspace = "' + (ci.vspace || 0) + '" />';
    ...
    (function() {
        var block = domUtils.isBlockElm, defaultValue = { left: 1, right: 1, center: 1, justify: 1 }, doJustify = function(range, style) {
            var bookmark = range.createBookmark(), filterFn = function(node) { return node.nodeType == 1 ? node.tagName.toLowerCase() != "br" && !domUtils.isBookmarkNode(node) : !domUtils.isWhitespace(node); };
            range.enlarge(true);
    ...
    (function() {
        var fonts = { "forecolor": "color", "backcolor": "background-color", "fontsize": "font-size", "fontfamily": "font-family", "underline": "text-decoration", "strikethrough": "text-decoration" };
        for (var p in fonts) {
            (function(cmd, style) {
                UE.commands[cmd] = {
    ...
    UE.commands["insertframe"] = { queryCommandState: function() { return this.highlight ? -1 : 0; } };
    UE.commands["removeformat"] = {
        execCommand: function(cmdName, tags, style, attrs, notIncludeA) {
            var tagReg = new RegExp("^(?:" + (tags || this.options.removeFormatTags).replace(/,/g, "|") + ")$", "i"), removeFormatAttributes = style ? [] : (attrs || this.options.removeFormatAttributes).split(","), range = new dom.Range(this.document), bookmark, node, parent, filter = function(node) { return node.nodeType == 1; };
    ...
    (function() {
        var getObj = function(editor) { return utils.findNode(editor.selection.getStartElementPath(), ["blockquote"]); };
            ...
            execCommand: function(cmdName, attrs) {
                ...
                if (obj) {
                    if (tds && tds.length) {
                        ...
                        var start = range.startContainer, startBlock = domUtils.isBlockElm(start) ? start : domUtils.findParent(start, function(node) { return domUtils.isBlockElm(node); }), end = range.endContainer, endBlock = domUtils.isBlockElm(end) ? end : domUtils.findParent(end, function(node) { return domUtils.isBlockElm(node); });
    ...
    UE.commands["indent"] = {
        ...
        queryCommandState: function() {
            ...
            var pN = utils.findNode(this.selection.getStartElementPath(), ["p", "h1", "h2", "h3", "h4", "h5", "h6"]), indent = pN && pN.style.textIndent ? parseInt(pN.style.textIndent) : "";
            return indent ? 1 : 0;
        }
    };
    ...
    UE.commands["preview"] = {
        execCommand: function() {
            var me = this, w = window.open("", "_blank", ""), d = w.document, css = me.document.getElementById("syntaxhighlighter_css"), js = document.getElementById("syntaxhighlighter_js"), style = "<style type='text/css'>" + me.options.initialStyle + "</style>", cont = me.getContent();
            if (browser.ie) {
    ...
    UE.plugins["selectall"] = function() {
        ...
        me.addListener("ready", function() { domUtils.on(me.document, "click", function(evt) { me.selectAll = false; }); });
    };
    (function() {
        var block = domUtils.isBlockElm, notExchange = ["TD", "LI", "PRE"], doParagraph = function(range, style, attrs, sourceCmdName) {
            var bookmark = range.createBookmark(), filterFn = function(node) { return node.nodeType == 1 ? node.tagName.toLowerCase() != "br" && !domUtils.isBookmarkNode(node) : !domUtils.isWhitespace(node); }, para;
            range.enlarge(true);
            var bookmark2 = range.createBookmark(), current = domUtils.getNextDomNode(bookmark2.start, false, filterFn), tmpRange = range.cloneRange(), tmpNode;
            while (current && !(domUtils.getPosition(current, bookmark2.end) & domUtils.POSITION_FOLLOWING)) {
                if (current.nodeType == 3 || !block(current)) {
    ...
    (function() {
        var block = domUtils.isBlockElm, getObj = function(editor) { return utils.findNode(editor.selection.getStartElementPath(), null, function(n) { return n.getAttribute("dir"); }); }, doDirectionality = function(range, editor, forward) {
            var bookmark, filterFn = function(node) { return node.nodeType == 1 ? !domUtils.isBookmarkNode(node) : !domUtils.isWhitespace(node); }, obj = getObj(editor);
    ...
    UE.commands["horizontal"] = {
        ...
        queryCommandState: function() { return this.highlight || utils.findNode(this.selection.getStartElementPath(), ["table"]) ? -1 : 0; }
    };
    ...
    UE.commands["rowspacing"] = {
        ...
        queryCommandValue: function(cmdName, dir) {
            var pN = utils.findNode(this.selection.getStartElementPath(), null, function(node) { return domUtils.isBlockElm(node); }), value;
            if (pN) {
    ...
    UE.commands["wordcount"] = {
        queryCommandValue: function(cmd, onlyCount) {
            ...
            var open = this.options.wordCount, max = this.options.maximumWords, msg = this.options.messages.wordCountMsg, errMsg = this.options.messages.wordOverFlowMsg;
            if (!open) {
                return "";
            }
            reg = new RegExp("[\r\t\n]", "g");
    ...
    UE.plugins["undo"] = function() {
        var me = this, maxUndoCount = me.options.maxUndoCount, maxInputCount = me.options.maxInputCount, fillchar = new RegExp(domUtils.fillChar + "|</hr>", "gi"), specialAttr = /\b(?:href|src|name)="[^"]*?"/gi;
        ...
        function UndoManager() {
            ...
            this.restore = function() {
                ...
                setTimeout(function() { range.scrollToView(me.autoHeightEnabled, me.autoHeightEnabled ? domUtils.getXY(me.iframe).y : 0); }, 200);
                this.update();
        ...
        me.commands["redo"] = me.commands["undo"] = { execCommand: function(cmdName) { me.undoManger[cmdName](); }, queryCommandState: function(cmdName) { return me.undoManger["has" + (cmdName.toLowerCase() == "undo" ? "Undo" : "Redo")] ? 0 : -1; }, notNeedUndo: 1 };
    ...
    (function() {
        ...
        UE.plugins["paste"] = function() {
            ...
            var modify_num = { flag: "" };
            me.commands["pasteplain"] = { queryCommandState: function() { return pasteplain; }, execCommand: function() { pasteplain = !pasteplain | 0; }, notNeedUndo: 1 };
                ...
                domUtils.on(me.body, browser.ie ? "keydown" : "paste", function(e) {
                    ...
                    getClipboardData.call(me, function(div) {
                        ...
                        if (modify_num.flag && me.ui) {
                            ...
                            modify_num.flag = "";
                            setTimeout(function() { me.addListener("selectionchange", hideMsg); }, 100);
                        }
    ...
    UE.plugins["list"] = function() {
        ...
        me.addListener("keydown", function(type, evt) {
            ...
            var keyCode = evt.keyCode || evt.which;
            if (keyCode == 13) {
                var range = me.selection.getRange(), start = domUtils.findParentByTagName(range.startContainer, ["ol", "ul"], true, function(node) { return node.tagName == "TABLE"; }), end = domUtils.findParentByTagName(range.endContainer, ["ol", "ul"], true, function(node) { return node.tagName == "TABLE"; });
        ...
        me.commands["insertorderedlist"] = me.commands["insertunorderedlist"] = {
            execCommand: function(command, style) {
                ...
                var me = this, range = this.selection.getRange(), filterFn = function(node) { return node.nodeType == 1 ? node.tagName.toLowerCase() != "br" : !domUtils.isWhitespace(node); }, tag = command.toLowerCase() == "insertorderedlist" ? "ol" : "ul", frag = me.document.createDocumentFragment();
                ...
                while (current && current !== bk.end && (domUtils.getPosition(current, bk.end) & domUtils.POSITION_PRECEDING)) {
                    if (current.nodeType == 3 || dtd.li[current.tagName]) {
                        ...
                        while (current && current !== bk.end && (!block(current) || domUtils.isBookmarkNode(current))) {
                            tmpNode = current;
                            current = domUtils.getNextDomNode(current, false, null, function(node) { return !notExchange[node.tagName]; });
                        }
            ...
            queryCommandState: function(command) { return this.highlight ? -1 : utils.findNode(this.selection.getStartElementPath(), [command.toLowerCase() == "insertorderedlist" ? "ol" : "ul"]) ? 1 : 0; },
    ...
    (function() {
        ...
        var sourceEditors = {
            textarea: function(editor, holder) {
                ...
                return {
                    setContent: function(content) { textarea.value = content; }, getContent: function() { return textarea.value; },
                    select: function() {
                        var range;
                        if (browser.ie) {
        ...
        UE.plugins["source"] = function() {
            ...
            me.commands["source"] = {
                ...
                queryCommandState: function() { return sourceMode | 0; }
            };
            ...
            me.addListener("ready", function() {
                if (opt.sourceEditor == "codemirror") {
                    utils.loadFile(document, { src: opt.codeMirrorJsUrl, tag: "script", type: "text/javascript", defer: "defer" });
                    utils.loadFile(document, { tag: "link", rel: "stylesheet", type: "text/css", href: opt.codeMirrorCssUrl });
                }
            });
        };
    })();
    UE.plugins["shortcutkeys"] = function() {
        var me = this, shortcutkeys = utils.extend({ "ctrl+66": "Bold", "ctrl+90": "Undo", "ctrl+89": "Redo", "ctrl+73": "Italic", "ctrl+85": "Underline", "ctrl+shift+67": "removeformat", "ctrl+shift+76": "justify:left", "ctrl+shift+82": "justify:right", "ctrl+65": "selectAll" }, me.options.shortcutkeys);
    ...
    UE.plugins["enterkey"] = function() {
        ...
        me.addListener("keyup", function(type, evt) {
            ...
            if (keyCode == 13) {
                ...
                setTimeout(function() { me.selection.getRange().scrollToView(me.autoHeightEnabled, me.autoHeightEnabled ? domUtils.getXY(me.iframe).y : 0); }, 50);
            }
        });
        me.addListener("keydown", function(type, evt) {
            var keyCode = evt.keyCode || evt.which;
            ...
            if (keyCode == 8) {
                ...
                if (!browser.ie) {
                    ...
                    if (browser.webkit && range.collapsed && start) {
                        ...
                        start = tmpRange.startContainer;
                        if (domUtils.isBlockElm(start) && start.nodeType == 1 && !dtd.$tableContent[start.tagName] && !domUtils.getChildCount(start, function(node) { return node.nodeType == 1 ? node.tagName !== "BR" : 1; })) {
                ...
                if (range.collapsed) {
                    ...
                    if (start && end && start === end) {
                        ...
                        if (start && end && start === end) {
                            ...
                            var bookmark = range.createBookmark(), filterFn = function(node) { return domUtils.isBlockElm(node); };
                            range.enlarge(true);
    ...
    UE.plugins["autolink"] = function() {
        ...
        var me = this;
        me.addListener("reset", function() { cont = 0; });
        me.addListener("keydown", function(type, evt) {
            var keyCode = evt.keyCode || evt.which;
    ...
    UE.plugins["autoheight"] = function() {
        ...
        me.enableAutoHeight = function() {
            ...
            setTimeout(function() { adjustHeight(); }, browser.gecko ? 100 : 0);
        ...
        me.addListener("ready", function() {
            ...
            domUtils.on(browser.ie ? me.body : me.document, browser.webkit ? "dragover" : "drop", function() {
                clearTimeout(timer);
                timer = setTimeout(function() { adjustHeight(); }, 100);
            });
        });
    };
    UE.plugins["autofloat"] = function() {
        var uiUtils, LteIE6 = browser.ie && browser.version <= 6, quirks = browser.quirks;
        ...
        var me = this, bakCssText, placeHolder = document.createElement("div"), toolbarBox, orgTop, getPosition, flag = true;
        ...
        var defer_updateFloating = utils.defer(function() { updateFloating(); }, browser.ie ? 200 : 100, true);
        me.addListener("destroy", function() {
        ...
        me.addListener("ready", function() {
            if (checkHasUI(me)) {
                ...
                me.addListener("sourcemodechanged", function(t, enabled) { setTimeout(function() { updateFloating(); }); });
            }
        });
    };
    UE.plugins["highlight"] = function() {
        ...
        me.commands["highlightcode"] = {
            ...
            queryCommandState: function() {
                ...
                start = domUtils.findParent(range.startContainer, function(node) { return node.nodeType == 1 && node.tagName == "DIV" && domUtils.hasClass(node, "syntaxhighlighter"); }, true);
                end = domUtils.findParent(range.endContainer, function(node) { return node.nodeType == 1 && node.tagName == "DIV" && domUtils.hasClass(node, "syntaxhighlighter"); }, true);
                return start && end && start == end ? 1 : 0;
            }
        };
        me.addListener("beforeselectionchange", function() { me.highlight = me.queryCommandState("highlightcode"); });
        me.addListener("afterselectionchange", function() { me.highlight = 0; });
        me.addListener("ready", function() {
            if (typeof XRegExp == "undefined") {
                var obj = { id: "syntaxhighlighter_js", src: me.options.highlightJsUrl, tag: "script", type: "text/javascript", defer: "defer" };
                utils.loadFile(document, obj, function() { changePre(); });
            }
            if (!me.document.getElementById("syntaxhighlighter_css")) {
                var obj = { id: "syntaxhighlighter_css", tag: "link", rel: "stylesheet", type: "text/css", href: me.options.highlightCssUrl };
        ...
        me.addListener("aftergetcontent", function(type, cmd) { changePre(); });
        ...
        me.addListener("aftersetcontent", function() { changePre(); });
    ...
    UE.plugins["serialize"] = function() {
        ...
        function ptToPx(value) { return /pt/.test(value) ? value.replace(/([\d.]+)pt/g, function(str) { return Math.round(parseFloat(str) * 96 / 72) + "px"; }) : value; }

        var me = this, EMPTY_TAG = dtd.$empty, parseHTML = function() {
                    ...
                    if ((tagName = match[3])) {
                        ...
                        if (attrPart) {
                            ...
                            while ((attrMatch = RE_ATTR.exec(attrPart))) {
                                ...
                                if (attrName == "style") {
                                    if (ie && version <= 6) {
                                        attrValue = attrValue.replace(/(?!;)\s*([\w-]+):/g, function(m, p1) { return p1.toLowerCase() + ":"; });
                                    }
                                }
                                if (attrValue) {
            ...
            return function(html, forceDtd) {
                ...
                parse(html, {
                    ...
                    onComment: function(text) { addChild({ type: "comment", data: text }); },
        ...
        var toHTML = function() {
                ...
                for (var k in attrs) {
                    ...
                    if (k == "style") {
                        ...
                        if (/rgba?\s*\([^)]*\)/.test(value)) {
                            value = value.replace(/rgba?\s*\(([^)]*)\)/g, function(str) { return utils.fixColor("color", str); });
                        }
            ...
            function printData(node, notTrans) { return notTrans ? node.data.replace(/&nbsp;/g, " ") : unhtml1(node.data).replace(/ /g, "&nbsp;"); }
        ...
        var transformWordHtml = function() {
            ...
            function ensureUnits(v) {
                v = v.replace(/([\d.]+)([\w]+)?/g, function(m, p1, p2) { return (Math.round(parseFloat(p1)) || 1) + (p2 || "px"); });
                return v;
            }
            ...
            function filterPasteWord(str) {
                ...
                }).replace(/v:\w+=["']?[^'"]+["']?/g, "").replace(/<(!|script[^>]*>.*?<\/script(?=[>\s])|\/?(\?xml(:\w+)?|xml|meta|link|style|\w+:\w+)(?=[\s\/>]))[^>]*>/gi, "").replace(/<p [^>]*class="?MsoHeading"?[^>]*>(.*?)<\/p>/gi, "<p><strong>$1</strong></p>").replace(/(lang)\s*=\s*([\'\"]?)[\w-]+\2/ig, "").replace(/<font[^>]*>\s*<\/font>/gi, "").replace(/class\s*=\s*["']?(?:(?:MsoTableGrid)|(?:MsoNormal(Table)?))\s*["']?/gi, "");
                ...
                str = str.replace(/([ ]+)<\/span>/ig, function(m, p) { return new Array(p.length + 1).join("&nbsp;") + "</span>"; });
                return str;
            }
        ...
        var first, orderStyle = { "decimal": /\d+/, "lower-roman": /^m{0,4}(cm|cd|d?c{0,3})(xc|xl|l?x{0,3})(ix|iv|v?i{0,3})$/, "upper-roman": /^M{0,4}(CM|CD|D?C{0,3})(XC|XL|L?X{0,3})(IX|IV|V?I{0,3})$/, "lower-alpha": /^\(?[a-z]+\)?$/, "upper-alpha": /^\(?[A-Z]+\)?$/ }, unorderStyle = { "disc": /^[l\u00B7\u2002]/, "circle": /^[\u006F\u00D8]/, "square": /^[\u006E\u25C6]/ }, parentTag = "", liStyle = "", firstTag;
        ...
        function Serialize(rules) { this.rules = rules; }
    ...
    UE.plugins["video"] = function() {
        ...
        me.addListener("beforegetcontent", function() { switchImgAndEmbed(true); });
        me.addListener("aftersetcontent", function() { switchImgAndEmbed(false); });
    ...
    UE.plugins["table"] = function() {
        ...
        function _isHide(cell) { return cell.style.display == "none"; }
        ...
        function _mouseDownEvent(type, evt) {
            ...
            if (me.queryCommandState("highlightcode") || domUtils.findParent(anchorTd, function(node) { return node.tagName == "DIV" && /highlighter/.test(node.id); })) {
                return;
            }
            if (evt.button == 2) {
                return;
    ...
    UE.plugins["contextmenu"] = function() {
        var me = this, menu, items = me.options.contextMenu;
                    ...
                    if (item == "-") {
                        ...
                        if (item.group) {
                                ...
                                (function(subItem) {
                                    if (subItem == "-") {
                                        ...
                                        if (me.queryCommandState(subItem.cmdName) != -1) {
                                            subMenu.push({ "label": subItem.label, className: "edui-for-" + subItem.cmdName + (subItem.value || ""), onclick: subItem.exec ? function() { subItem.exec.call(me); } : function() { me.execCommand(subItem.cmdName, subItem.value); } });
                                        }
                                    }
                                })(cj);
                            }
                            ...
                            if (me.queryCommandState(item.cmdName) != -1) {
                                ...
                                contextItems.push({ "label": item.label, className: "edui-for-" + (item.icon ? item.icon : item.cmdName + (item.value || "")), onclick: item.exec ? function() { item.exec.call(me); } : function() { me.execCommand(item.cmdName, item.value); } });
                            }
                        }
                    }
                })(ti);
        ...
        me.commands["formatmatch"] = {
            execCommand: function(cmdName) {
                ...
                if (!img || img.tagName != "IMG") {
                    ...
                    var start = range.startContainer;
                    list = domUtils.findParents(start, true, function(node) { return !domUtils.isBlockElm(node) && node.nodeType == 1; });
    ...
    UE.plugins["customstyle"] = function() {
            ...
            execCommand: function(cmdName, obj) {
                ...
                if (node && node.getAttribute("label") == obj.label) {
                    ...
                    if (range.collapsed) {
                        ...
                        node = domUtils.findParent(common, function(node) { return node.getAttribute("label") == obj.label; }, true);
                        if (node) {
                            domUtils.remove(node, true);
                        }
                    }
                    ...
                    if (dtd.$block[tagName]) {
                        ...
                        if (!range.collapsed) {
                            range.collapse();
                            node = domUtils.findParent(me.selection.getStart(), function(node) { return node.getAttribute("label") == obj.label; }, true);
                            var pNode = me.document.createElement("p");
        ...
        me.addListener("keyup", function(type, evt) {
            ...
            if (keyCode == 32 || keyCode == 13) {
                var range = me.selection.getRange();
                if (range.collapsed) {
                    var node = domUtils.findParent(me.selection.getStart(), function(node) { return node.getAttribute("label"); }, true);
    ...
    UE.plugins["catchremoteimage"] = function() {
        ...
        var ajax = UE.ajax, me = this, localDomain = me.options.localDomain, catcherUrl = me.options.catcherUrl, separater = "ue_separate_ue";

        function catchremoteimage(imgs, callbacks) {
            var submitStr = imgs.join(separater);
            ajax.request(catcherUrl, { content: submitStr, timeout: 60000, onsuccess: callbacks["success"], onerror: callbacks["error"] });
        }

        me.addListener("afterpaste", function() { me.fireEvent("catchRemoteImage"); });
    ...
    UE.commands["snapscreen"] = {
        execCommand: function() {
            var me = this, editorOptions = me.options;
            if (!browser.ie) {
            ...
            var onSuccess = function(rs) {
                ...
                me.execCommand("insertimage", { src: (editorOptions.snapscreenImgIsUseImagePath ? editorOptions.imagePath : "") + rs.url, floatStyle: editorOptions.snapscreenImgAlign, data_ue_src: (editorOptions.snapscreenImgIsUseImagePath ? editorOptions.imagePath : "") + rs.url });
            };
            var onStartUpload = function() {
            };
            var onError = function() { alert(editorOptions.messages.snapScreenMsg); };
            try {
    ...
    (function() {
        ...
        var uiUtils = baidu.editor.ui.uiUtils = {
            uid: function(obj) { return (obj ? obj[uidMagic] || (obj[uidMagic] = ++uidCount) : ++uidCount); },
            ...
            getViewportElement: function() { return (browser.ie && browser.quirks) ? document.body : document.documentElement; },
            ...
            unsetGlobal: function(id) { delete root[id]; },
            ...
            contains: function(elA, elB) { return elA && elB && (elA === elB ? false : (elA.contains ? elA.contains(elB) : elA.compareDocumentPosition(elB) & 16)); },
    ...
    (function() {
        ...
        UIBase.prototype = {
            ...
            initUIBase: function() { this._globalKey = utils.unhtml(uiUtils.setGlobal(this.id, this)); },
    ...
    (function() {
        ...
        Mask.prototype = {
            ...
            isHidden: function() { return this.getDom().style.display == "none"; },
            _onMouseDown: function() { return false; },
            _fill: function() {
                var el = this.getDom();
    ...
    (function() {
        ...
        Popup.prototype = {
            SHADOW_RADIUS: 5, content: null, _hidden: false, autoRender: true, canSideLeft: true, canSideUp: true,
            initPopup: function() {
                this.initUIBase();
                allPopups.push(this);
            },
            getHtmlTpl: function() { return '<div id="##" class="edui-popup %%">' + ' <div id="##_body" class="edui-popup-body">' + ' <iframe style="position:absolute;z-index:-1;left:0;top:0;background-color: white;" frameborder="0" width="100%" height="100%" src="javascript:"></iframe>' + ' <div class="edui-shadow"></div>' + ' <div id="##_content" class="edui-popup-content">' + this.getContentHtmlTpl() + "  </div>" + " </div>" + "</div>"; },
        ...
        domUtils.on(window, "scroll", function() { closeAllPopup(); });
    })();
    (function() {
        ...
        ColorPicker.prototype = {
            getHtmlTpl: function() { return genColorPicker(this.noColorText); },
    ...
    (function() {
        ...
        TablePicker.prototype = {
            ...
            _onMore: function() { this.fireEvent("more"); }
        };
    ...
    (function() {
        ...
        baidu.editor.ui.Stateful = {
            ...
            hasState: function(state) { return domUtils.hasClass(this.getStateDom(), "edui-state-" + state); },
    ...
    (function() {
        ...
        Button.prototype = {
            uiName: "button", label: "", title: "", showIcon: true, showText: true,
            initButton: function() {
                this.initUIBase();
                this.Stateful_init();
            },
    ...
    (function() {
        ...
        SplitButton.prototype = {
            popup: null, uiName: "splitbutton", title: "",
    ...
    (function() {
        ...
        ColorButton.prototype = {
            initColorButton: function() {
                var me = this;
                this.popup = new Popup({ content: new ColorPicker({ noColorText: "清除颜色", onpickcolor: function(t, color) { me._onPickColor(color); }, onpicknocolor: function(t, color) { me._onPickNoColor(color); } }), editor: me.editor });
                this.initSplitButton();
            },
            ...
            initTableButton: function() {
                ...
                this.popup = new Popup({
                    content: new TablePicker({
                        onpicktable: function(t, numCols, numRows) { me._onPickTable(numCols, numRows); },
                        onmore: function() {
                            me.popup.hide();
                            me.fireEvent("more");
                        }
    ...
    (function() {
        ...
        AutoTypeSetPicker.prototype = {
            ...
            getHtmlTpl: function() {
                var options = this.editor.options, opt = options.autotypeset;
    ...
    (function() {
        ...
        AutoTypeSetButton.prototype = {
            initAutoTypeSetButton: function() {
                var me = this;
                this.popup = new Popup({
                    content: new AutoTypeSetPicker({ editor: me.editor }), "editor": me.editor,
                    hide: function() {
                        if (!this._hidden && this.getDom()) {
                            getPara(this);
                            this.getDom().style.display = "none";
    ...
    (function() {
        ...
        Menu.prototype = {
            ...
            getSeparator: function() { return menuSeparator; },
            createItem: function(item) { return new MenuItem(item); },
        ...
        MenuItem.prototype = {
            label: "", subMenu: null, ownerMenu: null, uiName: "menuitem", alwalysHoverable: true, getHtmlTpl: function() { return '<div id="##" class="%%" stateful onclick="$$._onClick(event, this);">' + '<div class="%%-body">' + this.renderLabelHtml() + "</div>" + "</div>"; },
            postRender: function() {
                var me = this;
            ...
            delayShowSubMenu: function() {
                ...
                if (!me.isDisabled()) {
                    ...
                    clearTimeout(me._closingTimer);
                    me._closingTimer = null;
                    me._showingTimer = setTimeout(function() { me.showSubMenu(); }, 250);
                }
            },
    ...
    (function() {
        ...
        Dialog.prototype = {
            ...
            initDialog: function() {
                ...
                this.closeButton = new Button({ className: "edui-dialog-closebutton", title: "关闭对话框", onclick: function() { me.close(false); } });
                if (this.buttons) {
            ...
            getContentHtml: function() {
                ...
                if (typeof this.content == "string") {
                    ...
                    if (this.iframeUrl) {
                        contentHtml = '<iframe id="' + this.id + '_iframe" class="%%-iframe" height="100%" width="100%" frameborder="0" src="' + this.iframeUrl + '"></iframe>';
                    }
                }
                return contentHtml;
            },
            ...
            postRender: function() {
                ...
                this.addListener("hide", function() { me.modalMask.hide(); });
                if (this.buttons) {
                    for (var i = 0; i < this.buttons.length; i++) {
            ...
            _onCloseButtonClick: function(evt, el) { this.close(false); },
            close: function(ok) {
    ...
    (function() {
        ...
        while (k--) {
            ...
            editorui[cmd] = function(cmd) {
                return function(editor, title) {
                    ...
                    var ui = new editorui.Button({ className: "edui-for-" + cmd.toLowerCase(), title: title, onclick: function() { editor.execCommand(cmd); }, showText: false });
        ...
        editorui.SnapScreen = function(editor, title) {
            var cmd = "SnapScreen";
            title = title || editor.options.labelMap[cmd.toLowerCase()] || "";
            var ui = new editorui.Button({ className: "edui-for-" + cmd.toLowerCase(), title: title, onclick: function() { editor.execCommand(cmd); } });
            if (isIE) {
                var iframeUrl = editor.options.iframeUrlMap["snapscreen"];
                iframeUrl = editor.ui.mapUrl(iframeUrl);
                title = title || editor.options.labelMap["snapscreen"] || "";
                var dialog = new editorui.Dialog({
                    iframeUrl: iframeUrl, autoReset: true, draggable: true, editor: editor, className: "edui-for-snapscreen", title: title, buttons: [{ className: "edui-okbutton", label: "确认", onclick: function() { dialog.close(true); } }, { className: "edui-cancelbutton", label: "取消", onclick: function() { dialog.close(false); } }],
                    onok: function() {
                    },
                    oncancel: function() {
                    },
        ...
        editorui.ClearDoc = function(editor, title) {
            ...
            var ui = new editorui.Button({
                className: "edui-for-" + cmd.toLowerCase(), title: title,
                onclick: function() {
                    if (confirm("确定清空文档吗？")) {
        ...
        editorui.Justify = function(editor, side, title) {
            ...
            var ui = new editorui.Button({ className: "edui-for-justify" + side.toLowerCase(), title: title, onclick: function() { editor.execCommand("Justify", side); } });
            editor.addListener("selectionchange", function(type, causeByUi, uiReady) {
        ...
        editorui.JustifyLeft = function(editor, title) { return editorui.Justify(editor, "left", title); };
        editorui.JustifyCenter = function(editor, title) { return editorui.Justify(editor, "center", title); };
        editorui.JustifyRight = function(editor, title) { return editorui.Justify(editor, "right", title); };
        editorui.JustifyJustify = function(editor, title) { return editorui.Justify(editor, "justify", title); };
        editorui.ImageFloat = function(editor, side, title) {
            ...
            var ui = new editorui.Button({ className: "edui-for-image" + side.toLowerCase(), title: title, onclick: function() { editor.execCommand("imagefloat", side); } });
        ...
        editorui.ImageNone = function(editor, title) { return editorui.ImageFloat(editor, title); };
        editorui.ImageLeft = function(editor, title) { return editorui.ImageFloat(editor, "left", title); };
        editorui.ImageRight = function(editor, title) { return editorui.ImageFloat(editor, "right", title); };
        editorui.ImageCenter = function(editor, title) { return editorui.ImageFloat(editor, "center", title); };
        editorui.Directionality = function(editor, side, title) {
            side = (side || "left").toLowerCase();
            title = title || editor.options.labelMap["directionality" + side.toLowerCase()] || "";
            var ui = new editorui.Button({ className: "edui-for-directionality" + side.toLowerCase(), title: title, onclick: function() { editor.execCommand("directionality", side); }, type: side });
            editor.addListener("selectionchange", function(type, causeByUi, uiReady) {
        ...
        editorui.DirectionalityLtr = function(editor, title) { return new editorui.Directionality(editor, "ltr", title); };
        editorui.DirectionalityRtl = function(editor, title) { return new editorui.Directionality(editor, "rtl", title); };
            ...
            editorui[cmd] = function(cmd) {
                return function(editor, title) {
                    ...
                    var ui = new editorui.ColorButton({
                        className: "edui-for-" + cmd.toLowerCase(), color: "default", title: title, editor: editor, onpickcolor: function(t, color) { editor.execCommand(cmd, color); },
                        onpicknocolor: function() {
                            editor.execCommand(cmd, "default");
                            this.setColor("transparent");
                            this.color = "default";
                        },
                        onbuttonclick: function() { editor.execCommand(cmd, this.color); }
                    });
                    editor.addListener("selectionchange", function() {
        ...
        while (k--) {
            ...
            editorui[cmd] = function(cmd) {
                ...
                return function(editor, iframeUrl, title) {
                    ...
                    title = title || editor.options.labelMap[cmd.toLowerCase()] || "";
                    var dialog = new editorui.Dialog({
                        iframeUrl: iframeUrl, autoReset: true, draggable: true, editor: editor, className: "edui-for-" + cmd, title: title,
                        onok: function() {
                        },
                        oncancel: function() {
                        },
                    ...
                    dialog.render();
                    var ui = new editorui.Button({ className: "edui-for-" + cmd, title: title, onclick: function() { dialog.open(); } });
                    editor.addListener("selectionchange", function() {
                        var state = editor.queryCommandState(cmd);
                        if (state == -1) {
                            ui.setDisabled(true);
        ...
        while (k--) {
            ...
            editorui[cmd] = function(cmd) {
                ...
                return function(editor, iframeUrl, title) {
                    ...
                    title = title || editor.options.labelMap[cmd.toLowerCase()] || "";
                    var dialog = new editorui.Dialog({
                        iframeUrl: iframeUrl, autoReset: true, draggable: true, editor: editor, className: "edui-for-" + cmd, title: title, buttons: [{ className: "edui-okbutton", label: "确认", onclick: function() { dialog.close(true); } }, { className: "edui-cancelbutton", label: "取消", onclick: function() { dialog.close(false); } }],
                        onok: function() {
                        },
                        oncancel: function() {
                        },
        ...
        editorui.WordImage = function(editor) {
            var ui = new editorui.Button({
                className: "edui-for-wordimage", title: "图片转存",
                onclick: function() {
        ...
        editorui.FontFamily = function(editor, list, title) {
            ...
            for (var i = 0; i < list.length; i++) {
                ...
                items.push({ label: font, value: value, regex: regex, renderLabelHtml: function() { return '<div class="edui-label %%-label" style="font-family:' + utils.unhtml(this.value) + '">' + (this.label || "") + "</div>"; } });
            }
            var ui = new editorui.Combox({
                editor: editor, items: items, onselect: function(t, index) { editor.execCommand("FontFamily", this.items[index].value); }, onbuttonclick: function() { this.showPopup(); }, title: title, initValue: editor.options.ComboxInitial.FONT_FAMILY, className: "edui-for-fontfamily",
                indexByValue: function(value) {
                    if (value) {
        ...
        editorui.FontSize = function(editor, list, title) {
            ...
            for (var i = 0; i < list.length; i++) {
                var size = list[i] + "px";
                items.push({ label: size, value: size, renderLabelHtml: function() { return '<div class="edui-label %%-label" style="font-size:' + this.value + '">' + (this.label || "") + "</div>"; } });
            }
            var ui = new editorui.Combox({ editor: editor, items: items, title: title, initValue: editor.options.ComboxInitial.FONT_SIZE, onselect: function(t, index) { editor.execCommand("FontSize", this.items[index].value); }, onbuttonclick: function() { this.showPopup(); }, className: "edui-for-fontsize" });
            editor.addListener("selectionchange", function(type, causeByUi, uiReady) {
                if (!uiReady) {
        ...
        editorui.Paragraph = function(editor, list, title) {
            ...
            for (var i = 0; i < list.length; i++) {
                ...
                var tag = item[0];
                var label = item[1];
                items.push({ label: label, value: tag, renderLabelHtml: function() { return '<div class="edui-label %%-label"><span class="edui-for-' + this.value + '">' + (this.label || "") + "</span></div>"; } });
            }
            var ui = new editorui.Combox({ editor: editor, items: items, title: title, initValue: editor.options.ComboxInitial.PARAGRAPH, className: "edui-for-paragraph", onselect: function(t, index) { editor.execCommand("Paragraph", this.items[index].value); }, onbuttonclick: function() { this.showPopup(); } });
            editor.addListener("selectionchange", function(type, causeByUi, uiReady) {
                if (!uiReady) {
        ...
        editorui.CustomStyle = function(editor, list, title) {
            ...
            for (var i = 0, items = [], t; t = list[i++];) {
                (function(ti) { items.push({ label: ti.label, value: ti, renderLabelHtml: function() { return '<div class="edui-label %%-label">' + "<" + ti.tag + " " + (ti.className ? ' class="' + ti.className + '"' : "") + (ti.style ? ' style="' + ti.style + '"' : "") + ">" + ti.label + "</" + ti.tag + ">" + "</div>"; } }); })(t);
            }
            var ui = new editorui.Combox({
                editor: editor, items: items, title: title, initValue: editor.options.ComboxInitial.CUSTOMSTYLE, className: "edui-for-customstyle", onselect: function(t, index) { editor.execCommand("customstyle", this.items[index].value); }, onbuttonclick: function() { this.showPopup(); },
                indexByValue: function(value) {
        ...
        editorui.InsertTable = function(editor, iframeUrl, title) {
            ...
            var dialog = new editorui.Dialog({
                iframeUrl: iframeUrl, autoReset: true, draggable: true, editor: editor, className: "edui-for-inserttable", title: title, buttons: [{ className: "edui-okbutton", label: "确认", onclick: function() { dialog.close(true); } }, { className: "edui-cancelbutton", label: "取消", onclick: function() { dialog.close(false); } }],
                onok: function() {
                },
                oncancel: function() {
                },
            ...
            var ui = new editorui.TableButton({ editor: editor, title: title, className: "edui-for-inserttable", onpicktable: function(t, numCols, numRows) { editor.execCommand("InsertTable", { numRows: numRows, numCols: numCols }); }, onmore: function() { dialog.open(); }, onbuttonclick: function() { dialog.open(); } });
            editor.addListener("selectionchange", function() {
        ...
        editorui.AutoTypeSet = function(editor, iframeUrl, title) {
            ...
            var ui = new editorui.AutoTypeSetButton({ editor: editor, title: title, className: "edui-for-autotypeset", onbuttonclick: function() { editor.execCommand("autotypeset"); } });
            editor.addListener("selectionchange", function() { ui.setDisabled(editor.queryCommandState("autotypeset") == -1); });
            return ui;
        };
        editorui.LineHeight = function(editor, title) {
            for (var i = 0, ci, items = []; ci = editor.options.listMap.lineheight[i++];) {
                items.push({ label: ci == "1" ? "默认" : ci, value: ci, onclick: function() { editor.execCommand("lineheight", this.value); } });
            }
            var ui = new editorui.MenuButton({
                editor: editor, className: "edui-for-lineheight", title: title || editor.options.labelMap["lineheight"] || "", items: items,
        ...
        editorui.RowSpacingTop = function(editor, title) {
            for (var i = 0, ci, items = []; ci = editor.options.listMap.rowspacing[i++];) {
                items.push({ label: ci, value: ci, onclick: function() { editor.execCommand("rowspacing", this.value, "top"); } });
            }
            var ui = new editorui.MenuButton({
                editor: editor, className: "edui-for-rowspacingtop", title: title || editor.options.labelMap["rowspacingtop"] || "段前间距", items: items,
                onbuttonclick: function() {
        ...
        editorui.RowSpacingBottom = function(editor, title) {
            for (var i = 0, ci, items = []; ci = editor.options.listMap.rowspacing[i++];) {
                items.push({ label: ci, value: ci, onclick: function() { editor.execCommand("rowspacing", this.value, "bottom"); } });
            }
            var ui = new editorui.MenuButton({
                editor: editor, className: "edui-for-rowspacingbottom", title: title || editor.options.labelMap["rowspacingbottom"] || "段后间距", items: items,
                onbuttonclick: function() {
        ...
        editorui.InsertUnorderedList = function(editor, title) {
            ...
            var _onMenuClick = function() { editor.execCommand("InsertUnorderedList", this.value); };
            var ui = new editorui.MenuButton({
                editor: editor, className: "edui-for-insertunorderedlist", title: title, items: [{ label: "○ 小圆圈", value: "circle", onclick: _onMenuClick }, { label: "● 小圆点", value: "disc", onclick: _onMenuClick }, { label: "■ 小方块", value: "square", onclick: _onMenuClick }],
                onbuttonclick: function() {
        ...
        editorui.FullScreen = function(editor, title) {
            ...
            var ui = new editorui.Button({
                className: "edui-for-fullscreen", title: title,
                onclick: function() {
                    if (editor.ui) {
        ...
        editorui.Emotion = function(editor, iframeUrl, title) {
            ...
            iframeUrl = editor.ui.mapUrl(iframeUrl);
            var ui = new editorui.MultiMenuPop({ title: title, editor: editor, className: "edui-for-emotion", iframeUrl: iframeUrl });
            editor.addListener("selectionchange", function() {
    ...
    (function() {
        ...
        EditorUI.prototype = {
            ...
            initEditorUI: function() {
                ...
                this.initUIBase();
                this._initToolbars();
                var editor = this.editor, iframeUrlMap = editor.options.iframeUrlMap;
                editor.addListener("ready", function() {
                    baidu.editor.dom.domUtils.on(editor.window, "scroll", function() { baidu.editor.ui.Popup.postHide(); });
                ...
                editor.addListener("contextmenu", function(t, evt) { baidu.editor.ui.Popup.postHide(); });
                var me = this;
                editor.addListener("selectionchange", function() {
                    if (!editor.selection.isFocus()) {
                        return;
                ...
                var linkDialog = new baidu.editor.ui.Dialog({
                    iframeUrl: iframeUrlMap ? editor.ui.mapUrl(iframeUrlMap.link) : "", autoReset: true, draggable: true, editor: editor, className: "edui-for-link", title: "超链接", buttons: [{ className: "edui-okbutton", label: "确认", onclick: function() { linkDialog.close(true); } }, { className: "edui-cancelbutton", label: "取消", onclick: function() { linkDialog.close(false); } }],
                    onok: function() {
                    },
                    oncancel: function() {
                    },
                ...
                var anchorDialog = new baidu.editor.ui.Dialog({
                    iframeUrl: iframeUrlMap ? editor.ui.mapUrl(iframeUrlMap.anchor) : "", autoReset: true, draggable: true, editor: editor, className: "edui-for-anchor", title: "锚点", buttons: [{ className: "edui-okbutton", label: "确认", onclick: function() { anchorDialog.close(true); } }, { className: "edui-cancelbutton", label: "取消", onclick: function() { anchorDialog.close(false); } }],
                    onok: function() {
                    },
                    oncancel: function() {
                    },
                ...
                var tdDialog = new baidu.editor.ui.Dialog({
                    iframeUrl: iframeUrlMap ? me.mapUrl(iframeUrlMap["edittd"]) : "about:blank", autoReset: true, draggable: true, editor: editor, className: "edui-for-edittd", title: "单元格", buttons: [{ className: "edui-okbutton", label: "确认", onclick: function() { tdDialog.close(true); } }, { className: "edui-cancelbutton", label: "取消", onclick: function() { tdDialog.close(false); } }],
                    onok: function() {
                    },
                    oncancel: function() {
                    },
                ...
                var mapDialog = new baidu.editor.ui.Dialog({
                    iframeUrl: iframeUrlMap ? editor.ui.mapUrl(iframeUrlMap.map) : "", autoReset: true, draggable: true, editor: editor, className: "edui-for-map", title: "地图", buttons: [{ className: "edui-okbutton", label: "确认", onclick: function() { mapDialog.close(true); } }, { className: "edui-cancelbutton", label: "取消", onclick: function() { mapDialog.close(false); } }],
                    onok: function() {
                    },
                    oncancel: function() {
                    },
                ...
                var gmapDialog = new baidu.editor.ui.Dialog({
                    iframeUrl: iframeUrlMap ? editor.ui.mapUrl(iframeUrlMap.gmap) : "", autoReset: true, draggable: true, editor: editor, className: "edui-for-gmap", title: "Google地图", buttons: [{ className: "edui-okbutton", label: "确认", onclick: function() { gmapDialog.close(true); } }, { className: "edui-cancelbutton", label: "取消", onclick: function() { gmapDialog.close(false); } }],
                    onok: function() {
                    },
                    oncancel: function() {
                    },
                ...
                var popup = new baidu.editor.ui.Popup({
                    editor: editor, content: "", className: "edui-bubble",
                            ...
                            if (img && img.src.indexOf("http://api.map.baidu.com") != -1) {
                                ...
                                if (img && img.src.indexOf("http://maps.google.com/maps/api/staticmap") != -1) {
                                    ...
                                    if (img && img.getAttribute("anchorname")) {
                                        ...
                                        if (img && img.getAttribute("word_img")) {
                ...
                editor.addListener("selectionchange", function(t, causeByUi) {
                    ...
                    if (img != null && img.tagName.toLowerCase() == "img") {
                        if (img.getAttribute("anchorname")) {
                            html += popup.formatHtml('<nobr>属性: <span onclick=$$._onImgEditButtonClick(event) class="edui-clickable">修改</span>&nbsp;&nbsp;<span onclick=$$._onRemoveButtonClick(event) class="edui-clickable">删除</span></nobr>');
                        } else {
                            if (editor.options.imagePopup) {
                                html += popup.formatHtml('<nobr>属性: <span onclick=$$._onImgSetFloat("none") class="edui-clickable">默认</span>&nbsp;&nbsp;<span onclick=$$._onImgSetFloat("left") class="edui-clickable">居左</span>&nbsp;&nbsp;<span onclick=$$._onImgSetFloat("right") class="edui-clickable">居右</span>&nbsp;&nbsp;' + '<span onclick=$$._onImgSetFloat("center") class="edui-clickable">居中</span>' + ' <span onclick="$$._onImgEditButtonClick(event, this);" class="edui-clickable">修改</span></nobr>');
                            }
                        }
                    }
                    var link;
                    ...
                    if (link != null && (url = (link.getAttribute("data_ue_src") || link.getAttribute("href", 2))) != null) {
                        ...
                        html += popup.formatHtml('<nobr>链接: <a target="_blank" href="' + url + '" title="' + url + '" >' + txt + "</a>" + ' <span class="edui-clickable" onclick="$$._onEditButtonClick(event, this);">修改</span>' + ' <span class="edui-clickable" onclick="$$._onRemoveButtonClick(event, this);"> 清除</span></nobr>');
            ...
            getHtmlTpl: function() { return '<div id="##" class="%%">' + '<div id="##_toolbarbox" class="%%-toolbarbox">' + '<div id="##_toolbarboxouter" class="%%-toolbarboxouter"><div class="%%-toolbarboxinner">' + this.renderToolbarBoxHtml() + "</div></div>" + '<div id="##_toolbarmsg" class="%%-toolbarmsg" style="display:none;">' + '<div id = "##_upload_dialog" class="%%-toolbarmsg-upload" onclick="$$.showWordImageDialog();">点此上传</div>' + '<div class="%%-toolbarmsg-close" onclick="$$.hideToolbarMsg();">x</div>' + '<div id="##_toolbarmsg_label" class="%%-toolbarmsg-label"></div>' + '<div style="height:0;overflow:hidden;clear:both;"></div>' + "</div>" + "</div>" + '<div id="##_iframeholder" class="%%-iframeholder"></div>' + '<div id="##_bottombar" class="%%-bottomContainer"><table><tr>' + '<td id="##_elementpath" class="%%-bottombar"></td>' + '<td id="##_wordcount" class="%%-wordcount"></td>' + "</tr></table></div>" + "</div>"; },
            ...
            mapUrl: function(url) { return url.replace("~/", this.editor.options.UEDITOR_HOME_URL || ""); },
    ...
    (function() {
        ...
        MultiMenuPop.prototype = {
            initMultiMenu: function() {
                var me = this;
                this.popup = new Popup({
                    content: "", editor: me.editor, iframe_rendered: false,
                    onshow: function() {
