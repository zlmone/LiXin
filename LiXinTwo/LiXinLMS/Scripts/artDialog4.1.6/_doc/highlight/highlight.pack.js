...
var hljs = new function() {
    ...
    function nodeStream(node) {
        ...
        (function(node, offset) {
            for (var i = 0; i < node.childNodes.length; i++) {
                if (node.childNodes[i].nodeType == 3) offset += node.childNodes[i].nodeValue.length;
                else if (node.childNodes[i].nodeName == 'BR') offset += 1;
                else {
    ...
    function mergeStreams(stream1, stream2, value) {
        ...
        function selectStream() {
            if (stream1.length && stream2.length) {
                ...
                else return (stream1[0].event == 'start' && stream2[0].event == 'stop') ? stream2 : stream1;
            } else {
    ...
    function highlightBlock(block, tabReplace) {
        ...
        if (result) {
            ...
            ;
            if (tabReplace) {
                result = result.replace(/^((<[^>]+>|\t)+)/gm, function(match, p1, offset, s) { return p1.replace(/\t/g, tabReplace); });
            }
            ;
