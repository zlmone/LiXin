    ...
    switch (type) {
        ...
        case 5:
            //填空
            {
                ...
                an = (an==""?an:an.substring(6, an.length));
            }
            break;
        case 6:
            //情景
...
Exam.SubmitExam = function (str) {
    ...
    if (str != undefined && str != "") {
        //时间到提交
        $.post("/ExamTest/SubmitStudentAnswer?euid=" + $("#examUserID").val()
            + "&submitType=2&courseType=" + $("#txtsourceType").val() + "&pecent=" + Exam.Examination.PercentScore + "&passScore=" + Exam.Examination.PassScore, $("#submitAnswerForm").formSerialize(), function (data)
            {
            if (data.result == 1) {
                ...
                clearInterval(timeExamInterval);
                if (data.pass == 1) {
                    art.dialog.tips("恭喜，您已通过本次测试，可查看获取学时情况", 3, function () { window.location.href = getQueryString("backurl") + "&tp=" + getQueryString("tp") + "&backFrom=" + getQueryString("backFrom") });
                    //window.location.href = getQueryString("backurl");
                }
                else if (data.pass == 0) {
                    art.dialog.tips("抱歉，您未能通过本次测试，本课程还剩余" + data.nopassnumber + "次考试机会", 3, function () { window.location.href = getQueryString("backurl") + "&tp=" + getQueryString("tp") + "&backFrom=" + getQueryString("backFrom") });
                }
                else if (data.pass == 2) {
                    art.dialog.tips("恭喜，您已通过本次测试。", 3, function () { window.location.href = getQueryString("backurl") + "&tp=" + getQueryString("tp") + "&backFrom=" + getQueryString("backFrom") });
                }
            }
