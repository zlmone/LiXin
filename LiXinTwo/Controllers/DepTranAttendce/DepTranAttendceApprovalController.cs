...
namespace LiXinControllers.DepTranAttendce
{
    public partial class DepTranAttendceController : BaseController
    {
        ...
        public ActionResult DepTranAttendceApprovalList()
        {
            return View();
        }
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult DepTranAttendceApprovalCourseInfor()
        {
            ...
            var att = depOpenBL.GetCourseOpen(Convert.ToInt32(aa.Split('㉿')[0]), Convert.ToInt32(aa.Split('㉿')[1]));

            ViewBag.att = att.ApprovalFlag;
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult DepTranAttendceApprovalAllowed()
        {
        ...
        public ActionResult DepTranAttendceApprovalResource(int courseId, int deptId)
        {
            List<DepTran_CourseAttFile> resource = new List<DepTran_CourseAttFile>();
            resource = depAttFileBL.GetDepTran_CourseAttFileDownload(courseId, deptId, " IsDelete=0 ");
            ViewBag.resource = resource;
            return View();
        }

        public ActionResult DepTran_AttendceApprovalRecord()
        {
            return View();
        }
        #endregion
        ...
        /// <returns></returns>
        public JsonResult GetDepTranAttendceApprovalList(int kind, string courseName, string depName, int ismust, int isopen, int appFlag, string startTime, string endTime, int pageSize = 20, int pageIndex = 1)
        {
            ...
            {
                where += string.Format(" and temp.DeptName like '%{0}%' ", depName.ReplaceSql());
                whereFlag += string.Format(" and d.DepartSetName like '%{0}%' ", depName.ReplaceSql());
            }
            if (ismust != -1)
            {
            ...
            var list = Approval.GetDepTranAttendceApprovalList(whereFlag, pageIndex, pageSize, order, where);
        ...
        /// <param name="pageIndex">当前页</param>
        /// <returns></returns>
        public JsonResult GetDepTranAttendceUserList(string realName, int isMust, int isOpen, int courseId, string deptId, int pageSize = 20, int pageIndex = 1)
        {
            var where = " 1=1 ";
            ...
            //if (isOpen != -1)
            //{
            //    where += string.Format(" and dta.ApprovalFlag={0}", isOpen);
            //}
            int totalCount = 0;
            var list = depAttBL.GetDepAttendUserList(courseId, deptId.StringToInt32(), out totalCount, pageIndex, pageSize, "asc", where);
            //var list = Approval.GetDepTran_AttendceUserList(courseId, deptId, pageIndex, pageSize, "asc", where);
            int n = 0;
        ...
        /// 更新审批状态
        /// </summary>
        public string GetDepTranAttendceUser(int courseId, int userId, string reason, int approval, decimal attScore)
        {
            //var isDone = Approval.DepTranAttendceApprovalUsers(courseId, userId, reason, approval);
            var isScore = depOrderBL.DepTran_CourseOrderUsers(courseId, userId, attScore);
            if (isScore == true)
            {
                return "0";
            }
        ...
        public bool DataDeal(int courseId, int departId, int type)
        {
            ...
            decimal score;
            var list = Approval.GetUserList(courseId, departId);
            int n = 0;
            var itemArray = new object[list.Count()];
            var isScore = false;
            foreach (var item in list)
            {
                ...
                {
                    Status = item.Status,
                    ApprovalFlagStr = item.attendStatusStr,
                    UserId = item.UserId,
                    CourseId = item.CourseId,
                    ApprovalFlag = item.AttFlag,
                    deptId = item.DepartID,
                ...
                if (type == 0)
                {
                    depAttBL.UpdatePersonalAttendce(item.CourseId, item.UserId, item.DepartSetId);
                }
        ...
        {
            var list = depOpenBL.GetCourseOpen(courseId, departId);
            ...
            {
                reasult = 1;
                isApproval = depOpenBL.UpDateDepTran_CourseOpen(courseId, departId, 1, 1);
            }
            else
            {
                reasult = 2;
                isApproval = depOpenBL.UpDateDepTran_CourseOpen(courseId, departId, 0, 1);
            }
            if (isApproval == true)
            {
        ...
        /// 发送短信and邮件
        /// </summary>
        [Filter.SystemLog("转播考勤审批", LogLevel.Info)]
        public JsonResult SendEmailMessage(int courseId, int departId, int type, int sendMessage)
        {
            ...
            try
            {
                ...
                {
                    var course = depOpenBL.CourseName(courseId, departId);
                    var userInfo = depOpenBL.UserNmae(courseId, departId);
        ...
        public void LoadPrincipleFile(string path, string name)
        {
            if (System.IO.File.Exists(Server.MapPath(DepTranFileUrl + path)))
            {
                var filePath = Server.MapPath(DepTranFileUrl + path); //路径 
                //以字符流的形式下载文件
