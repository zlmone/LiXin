...
namespace LiXinControllers.DepTranAttendce
{
    public partial class DepTranAttendceController : BaseController
    {
        protected IDepTran_Attendce depAttBL;
        protected IDepTran_DepartLeaderConfig depLConfigBL;
        protected IUser userBL;
        protected ICo_Course coBL; 
        protected IDepTranAttendceApproval Approval;
        protected IDepTran_CourseOrder depOrderBL;
        protected IDepTran_CourseOpen depOpenBL;
        protected IDepTran_CourseAttFile depAttFileBL;
        protected IDepTran_AttendceApprovalRecord depAttRecordBL;
        protected ISys_Leader leaderBL;

        public DepTranAttendceController(IDepTran_Attendce _depAttBL, IDepTran_DepartLeaderConfig _depLConfigBL, IUser _userBL, IDepTranAttendceApproval approval, IDepTran_CourseOrder _depOrderBL, IDepTran_CourseOpen _depOpenBL, IDepTran_CourseAttFile _depAttFileBL, IDepTran_AttendceApprovalRecord _depAttRecordBL, ISys_Leader _leaderBL, ICo_Course _coBL)
        {
            depAttBL = _depAttBL;
            depLConfigBL = _depLConfigBL;
            userBL = _userBL;
            Approval = approval;
            depOrderBL = _depOrderBL;
            depOpenBL = _depOpenBL;
        ...
        ///  考勤列表呈现
        /// </summary>
        public ViewResult DepTranAttendceList()
        {
            int uid = CurrentUser == null ? 0 : CurrentUser.UserId;
            List<DepTranDepartSetting> depConfiglist = depLConfigBL.GetDepartSettingByUserId(uid);
            if (depConfiglist.Count > 0)
            {
                ViewBag.did = depConfiglist[0].Id;
            }
            else
            {
                ViewBag.did = 0;
        ...
        /// </summary>
        /// <returns></returns>
        public ViewResult DepTranAttendce(int id, int did)
        {
            ViewBag.flag = 0;
            ...
            if (times != "-1")
            {
                Co_Course comodel=coBL.GetCo_Course(id);
        ...
        /// </summary>
        /// <returns></returns>
        public ViewResult AttendceStatus(int cid, int uid,int did,int IsAttFlag,string uids)
        {
            ViewBag.cid=cid;
            ViewBag.uid = uid;
            ViewBag.did = did;
        ...
        /// 导入考勤呈现
        /// </summary>
        public ActionResult ImportDepTranAttend()
        {
            return View();
        }
        /// <summary>
        ...
        /// </summary>
        /// <returns></returns>
        public JsonResult GetDeptAttendceList(int did,string courseName, string isMust, string isOpen, int attState, int CoState, string StartTime, string EndTime, int pageSize = 20, int pageIndex = 1)
        {
            if (did > 0)
            {
                string where = "1=1";
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(isOpen))
                    {
                        if (!isOpen.Contains(","))
                        {
                            where += string.Format(" AND temp.OpenFlag={0}", Convert.ToInt32(isOpen));
                        }
                    }
                    ...
                    List<AttendceCourseModel> deptattendceList = depAttBL.GetDepAttendceList(did.ToString(), out totalCount, pageIndex, pageSize, "StartTime DESC", where);
        ...
        public JsonResult GetDeptAttendUserList(int id, int deptId, string uname, int AState, int pageSize = 20, int pageIndex = 1)
        {
            try
            {
                ...
                List<DepTran_CourseOrder> attendceList = depAttBL.GetDepAttendUserList(id, deptId,out totalCount, pageIndex, pageSize, "asc", where);
        ...
        public JsonResult GetDeptAttendRecordList(int id, int deptId, int pageSize = 20, int pageIndex = 1)
        {
            ...
            {
                int totalCount = 0;
                List<DepTran_AttendceApprovalRecord> attendceRecordList = depAttRecordBL.GetDepTranApprovalRecordlist(id, deptId, out totalCount, pageIndex, pageSize);
        ...
        public void OutTemplate()
        {
            ...
            new Spreadsheet().Template(null, null, outTable, HttpContext, "转播考勤模板", "考勤模板");
        }
        ...
        /// </summary>
        /// <returns></returns>
        public JsonResult SubmitImportAtt(int cid, int did)
        {
                string filename = "";
                string resultName = "";
                                ...
                                for (int j = 0; j < dtList[i].Rows.Count; j++)
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(uid) && !string.IsNullOrEmpty(stat))
                                    {
                                        ...
                                        if (departsetid > 0)
                                        {
                                            if (stat == "正常")
                                            {
                                                UpdateStatus(cid, userid, departsetid, 1);
                                                cg++;
                                            }
                                            else if (stat == "缺勤")
                                            {
                                                UpdateStatus(cid, userid, departsetid, 2);
                                                cg++;
                                            }
                                            else if (stat == "迟到")
                                            {
                                                UpdateStatus(cid, userid, departsetid, 3);
                                                cg++;
                                            }
                                            else if (stat == "早退")
                                            {
                                                UpdateStatus(cid, userid, departsetid, 4);
                                                cg++;
                                            }
                                            else if (stat == "迟到且早退")
                                            {
                                                UpdateStatus(cid, userid, departsetid, 5);
                                                cg++;
                                            }
                                            else
                                            {
        ...
        public JsonResult SubmitFile(int cid, int did,string file)
        {
            try
            {
                ...
                for (int i = 0; i < files.Length; i++)
                {
                    ...
                    var attFile = new DepTran_CourseAttFile
                    {
                        CourseId = cid,
                        DepartId = did,
                        Type = 0,
        ...
        public JsonResult AttendEnd(int courseid, int did)
        {
            ...
            try
            {
                int sum=depAttBL.ValidaDepAttStatus(courseid, did);
                if (sum < 1)
                {
                    var model = depOpenBL.GetCourseOpen(courseid, did);
                    if (model != null)
                    {
                        model.AttFlag = 1;
                        ...
                        depOpenBL.UpdateCourseOpen(model);
                    }
                    else
                    {
                        var open = new DepTran_CourseOpen
                        {
                            CourseId = courseid,
                            DepartId = did,
                            AttFlag = 1,
                        ...
                        depOpenBL.AddDepCourseOpen(open);
                    }
                    return Json(new
                    {
                        result = 1
        ...
        public JsonResult GetUserInfoList(int id,int did, string realName, string deptName, int sex, int pageSize = 20, int pageIndex = 1)
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(deptName))
                    where += string.Format(" and su.DeptName like '%{0}%' ", deptName.ReplaceSql());
                if (!string.IsNullOrWhiteSpace(realName))
                    where += string.Format(" and su.Realname like '%{0}%' ", realName.ReplaceSql());
                if (sex != 99)
                    where += " and su.Sex = " + sex;

                var list = depAttBL.GetDepartUserList(id, did, out totalCount, pageIndex, pageSize, where);
        ...
        /// <param name="courseid"></param>
        /// <returns></returns>
        public JsonResult SubmitUser(int courseid, string userids)
        {
            try
            {
                int a = 0;
                ...
                for (int i = 0; i < userid.Length; i++)
                {
                    ...
                    var course = new DepTran_CourseOrder
                    {
                        CourseId = courseid,
                        ...
                        EvlutionScore=0,
                        ExamScore=0,
                        DepartSetId=Convert.ToInt32(us[1])
                    };
                    bool ret=depOrderBL.AddDepTran_CourseOrder(course);
                    if (ret)
                    {
                        var model=depAttBL.GetDepAttendce(courseid, Convert.ToInt32(us[0]));
                        if (model == null)
                        {
                            var attendce = new DepTran_Attendce
                            {
                                CourseId = courseid,
                                UserId = Convert.ToInt32(us[0]),
        ...
        /// <returns></returns>
        public JsonResult SubmitStatus(int courseid, int userid, int departSetId, int isAttFlag, string uids)
        {
            try
            {
                if (uids == "0" && userid > 0 && departSetId > 0)
                {
                    UpdateStatus(courseid, userid, departSetId, isAttFlag);
                    return Json(new
                    {
                        result = 1
                    }, JsonRequestBehavior.AllowGet);
                ...
                else
                {
                    ...
                    for (int i = 0; i < uid.Length; i++)
                    {
                        ...
                        UpdateStatus(courseid, Convert.ToInt32(us[0]), Convert.ToInt32(us[1]), isAttFlag);
                    }
                    return Json(new
                    {
                        result = 1
        ...
        public void UpdateStatus(int courseid, int userid, int departSetId, int isAttFlag)
        {
            var coursemodel = depOrderBL.GetCo_CourseMainPaperByCourseIdAndUserId(courseid, userid);
            if (coursemodel == null)
            {
                var course = new DepTran_CourseOrder
                {
                ...
                depOrderBL.AddDepTran_CourseOrder(course);
            }
            else
            {
                if (coursemodel.OrderStatus == 0 || coursemodel.OrderStatus == 3)
                {
                    coursemodel.OrderStatus = 2;
                    depOrderBL.UpdateDepTran_CourseOrder(coursemodel);
                }
            }
        ...
        /// </summary>
        /// <returns></returns>
        public JsonResult GetDifferenceList(int did,string courseName,int pageSize = 20, int pageIndex = 1)
        {
            if (did > 0)
            {
                string where = "1=1";
                try
                {
                    ...
                    List<AttendceCourseModel> differenceList = depAttBL.GetDifferenceList(did, out totalCount, pageIndex, pageSize, "cc.StartTime DESC", where);
        ...
        /// 下载差异表
        /// </summary>
        public void OutDiffer(int did,string courseName)
        {
            if (did > 0)
            {
                int totalCount = 0;
                ...
                List<AttendceCourseModel> differenceList = depAttBL.GetDifferenceList(did, out totalCount, 1, int.MaxValue, "cc.StartTime DESC", where);
                DataTable outTable = new DataTable();
                ...
                foreach (AttendceCourseModel model in differenceList)
                {
                    outTable.Rows.Add(model.CourseName, model.DepartSetName, model.CoCount, model.AttCount, model.AttOkCount, model.AttNoCount, model.NoAttCount, model.chidaoCount, model.zaotuiCount, model.chizaoCount);
                }
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult GetUserdept()
        {
            int uid = CurrentUser == null ? 0 : CurrentUser.UserId;
            List<DepTranDepartSetting> depConfiglist = depLConfigBL.GetDepartSettingByUserId(uid);
            ...
            treeStr.Append("<ul>");
            foreach (DepTranDepartSetting dept in depConfiglist)
            {
                treeStr.AppendFormat(
                    "<li id='{0}' class='pNote'><a title=\"{1}\" onclick=\"selectAtt({0},this);\" id=\"tree{0}\">{1}</a>",
                    dept.Id, dept.DepartSetName);
                treeStr.Append("</li>");
            }
            treeStr.Append("</ul>");
            treeStr.Append("</li>");
