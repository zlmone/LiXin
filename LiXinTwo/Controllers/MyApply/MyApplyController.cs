...
namespace LiXinControllers
{
    public partial class MyApplyController : BaseController
    {
        ...
        protected IFree_UserApplyFiles ifree_userapplyfiles;
        protected IFree_UserOtherApply iFree_UserOtherApply;
        protected IFree_UserApplyTeacherDetails iFree_UserApplyTeacherDetails;
        protected IFree freeBL;
        private IFree_UserApplyTeacherDetails teacherDetailsBL;
        private IFree_UserApplyOtherForm free_UserApplyOtherBL;
        private IFree_OtherFroms fFree_otherfrom;
        private IFree_AllOtherApply iFree_AllOtherApplyBL;
        protected IUser userBL;
        protected IReport_Free reportFreeBL;
        public MyApplyController(ICourseOrder _courseOrderBL, IMyApproval _myAppBL, IFreeConfig _freeConfigBL, ITr_YearPlan _Iyear, IFree_UserApplyFiles _ifree_userapplyfiles,IFree_UserOtherApply _iFree_UserOtherApply,
            IFree_UserApplyTeacherDetails _iFree_UserApplyTeacherDetails, IFree _freeBL, IFree_UserApplyTeacherDetails _teacherDetailsBL,
            IFree_UserApplyOtherForm _free_UserApplyOtherBL, IFree_OtherFroms _fFree_otherfrom,
            IFree_AllOtherApply _iFree_AllOtherApplyBL, IUser _userBL, IReport_Free _reportFreeBL)
        {
            courseOrderBL = _courseOrderBL;
            myAppBL = _myAppBL;
            freeConfigBL = _freeConfigBL;
            Iyear = _Iyear;
            ifree_userapplyfiles = _ifree_userapplyfiles;
            iFree_UserOtherApply = _iFree_UserOtherApply;
            iFree_UserApplyTeacherDetails = _iFree_UserApplyTeacherDetails;
            freeBL = _freeBL;
            teacherDetailsBL = _teacherDetailsBL;
            free_UserApplyOtherBL = _free_UserApplyOtherBL;
            fFree_otherfrom = _fFree_otherfrom;
            iFree_AllOtherApplyBL = _iFree_AllOtherApplyBL;
            userBL = _userBL;
        ...
        public JsonResult GetMyComplementResList(string course, int Approval, int pageSize = 20, int pageIndex = 1, string jsRenderSortField = "Id desc")
        {
            ...
            foreach (var item in list)
            {
                ...
                var temp = new
                {
                    ...
                    TotalCourseTimes = item.TotalCourseTimes,
                    TeacherName = item.TeacherName.HtmlXssEncode(),
                    GetScore = item.GetScore < 0 ? 0 : item.GetScore,
        ...
        /// <returns></returns>
        [Filter.SystemLog("逾时申请审批管理 审批 补预订", LogLevel.Info)]
        ...
        /// <param name="id"></param>
        /// <returns></returns>
         [Filter.SystemLog("逾时申请审批管理 审批 请假", LogLevel.Info)]
        ...
        public JsonResult GetTimeOutLeave(int type = 99, int approval = 99, string realname = "", string traingrade = "", string applyname = "", string applydept = "", int pageSize = 10, int pageIndex = 1, string jsRenderSortField = " Id desc")
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(traingrade))
                    where += string.Format(" and ('{0}' like '%'+a.TrainGrade+'%'  and (a.traingrade is not null and a.traingrade <> ''))", traingrade);
                if (approval != 99)
                {
