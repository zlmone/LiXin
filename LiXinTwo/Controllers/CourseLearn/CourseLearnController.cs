...
", CurrentUser.UserId, courseType, iscpa, IsOpenSub, openlevel);
        ...
        public JsonResult GetMyCPACourseSubscribeList(string course, int subscribeType, DateTime? start, DateTime? end, int pageSize = 20, int pageIndex = 1, string jsRenderSortField = "Id desc")
        {
            ...
            int totalCount = 0;
            string where = string.Format(@" IsDelete = 0 
and Publishflag = 1 
and CourseFrom = 2 
and Way = 3
and (
    ((select count(*) from Sys_User where UserId={0} and TrainGrade in (select id from dbo.F_SplitIDs(OpenLevel)))>0  
            or OpenLevel = '' or OpenLevel is null ) 
    and 
    ((select count(*) from Sys_GroupUser where UserId={0} and groupid in 
        (select id from dbo.F_SplitIDs(OpenGroupObject)))>0
	    or OpenGroupObject = '' or OpenGroupObject is null)
    )
", CurrentUser.UserId);
            if (!string.IsNullOrWhiteSpace(course))
                where += string.Format(" and CourseName like '%{0}%'", course.ReplaceSql());
            if (start.HasValue)
                where += string.Format(" and StartTime >= '{0}'", start.Value);
            if (end.HasValue)
                where += string.Format(" and EndTime <= '{0}'", end.Value.AddDays(1));
            switch (subscribeType)
            {
                case 0://待预订
                    where += string.Format(" and cluserID is null  AND  HasEntered<>NumberLimited and StartTime >= '{0}'", DateTime.Now);
                    break;
                case 1://预订成功
                    where += " and cluserID > 0 ";
                    break;
                case 2://已过期
                    where += string.Format(" and cluserID is null  AND  HasEntered<>NumberLimited and StartTime < '{0}'", DateTime.Now);
                    break;
                case 3://人员已满
                    ...
                    break;
                default:
                    where += " and cluserID is null ";
                    break;
            }
         
            var list = courseOrderBL.GetMyCourseCPAList(out totalCount, CurrentUser.UserId, where, pageIndex, pageSize,jsRenderSortField);
            int n = 0;
            var itemArray = new object[list.Count];
            foreach (var item in list)
            {
                var temp = new
                {
                    ...
                    MyStatusStr = item.MyStatus == 0 ? (item.HasEntered == item.NumberLimited ? "人员已满" : "待预订") : "已过期",
                    NumberLimited = item.NumberLimited,
                    HasEntered = item.HasEntered
                };
                itemArray[n] = temp;
...
", courseId);
        ...
        public JsonResult GetCPACourseSubscribeUserList(int id, string pname = "", string dname = "", int flag = -1, int pageSize = 20, int pageIndex = 1, string jsRenderSortField = "UserId desc")
        {
            ...
            if (!string.IsNullOrWhiteSpace(dname))
                where += string.Format(" and su.DeptPath like '%{0}%'", dname.ReplaceSql());
            if (!string.IsNullOrWhiteSpace(pname))
                where += string.Format(" and su.RealName like '%{0}%'", pname.ReplaceSql());
            if (flag != -1)
            {
...
", courseId, leaderId, appoint, appoint == 0 ? "and cl_courseorder.orderstatus <> 0" : "", (appoint == 3 ? "or cl_courseorder.orderstatus=3 " : "")
        ...
        /// 获取集中授课中报名的人员
        /// </summary>
        public JsonResult GetCourseOrderName(int courseID, string realName = "", string deptName = "", int pageSize = 10, int pageIndex = 1, string jsRenderSortField = " su.UserId asc")
        {
            try
            {
                int totalCount = 0;
                ...
                var list = courseOrderBL.GetCourseOrderName(out totalCount, courseID, where, pageSize, pageIndex, jsRenderSortField);
                return Json(new
                      {
                          result = 1,
                          dataList = list,
