...
namespace LiXinControllers
{
    ...
    public partial class MyApprovalController : BaseController
    {
        ...
        private IFreeConfig FreeConfigBL;
        protected IFree_UserOtherApply iFree_UserOtherApply;
        protected IFree_UserApplyOtherForm iFree_UserApplyOtherForm;
        /// <summary>
        /// 构造函数
        /// </summary>
        /// <param name="approval"></param>
        public MyApprovalController(IMyApproval approval, ISys_TrainGrade trainGrade, ICo_Course courseBL, ISys_Leader _leaderBL,
            ICl_CpaLearnStatus cpaLearn, ICl_Attendce _IAttendce, IFree _freeBL, IFree_UserApplyTeacherDetails _teacherDetailsBL, IFreeConfig _FreeConfigBL, IFree_UserOtherApply _iFree_UserOtherApply, IFree_UserApplyOtherForm _iFree_UserApplyOtherForm)
        {
            Approval = approval;
        ...
        public JsonResult GetMyApprovalPrincipleList(string courseName = "", string realName = "", string trainGrade = "", int status = -1, string startTime = "2000-1-1", string endTime = "2050-1-1", int pageIndex = 1, int pageSize = 20, string jsRenderSortField = "StartTime desc")
        {
            ...
            list = Approval.GetMyApprovalPrinciplePagingForNew(out totalCount, sqlwhere, pageIndex, pageSize, sqlorderby);
        ...
        public JsonResult GetMyPrincipleList(string courseName = "", int ast = -1, int aat = -1, int aast = -1, string startTime = "2000-1-1", string endTime = "2050-1-1", int pageIndex = 1, int pageSize = 20, string jsRenderSortField = "StartTime desc")
        {
            ...
            else if (ast == 0)//迟到
            {
                aststr += @" and
                (
                   (b.AttFlag=3 and a.StartTime>dateadd(n,1,b.StartTime) and a.EndTime>=dateadd(n,1,b.EndTime)) or
                   (b.AttFlag=1 and a.StartTime>dateadd(n,1,b.StartTime))
                )  and a.LessLength<0    ";
            }
            else if (ast == 1)//早退
            {
                aststr += @" and
                (
                   (b.AttFlag=3 and a.EndTime<dateadd(n,1,b.EndTime) and a.StartTime<=dateadd(n,1,b.StartTime)) or 
                   (b.AttFlag=2 and a.EndTime<dateadd(n,1,b.EndTime))
                )  and a.LessLength<0";
            }
            else if (ast == 3)//迟到且早退
            {
                aststr += aststr += @" and
                (
                   b.AttFlag=3 and a.StartTime>dateadd(n,1,b.StartTime) and a.EndTime<dateadd(n,1,b.EndTime)
                )  and a.LessLength<0";
            }
            //var sqlwhere = string.Format("{0} {1} {2} {3} {4} {5} ",
            //                             courseName.Trim() == ""
            ...
            if (aast != -1)
            {
                if (aast == 3)
                {
                    sqlwhere += "and a.IsApp=0";
                }
                else
                {
                    sqlwhere += string.Format(" and a.ApprovalFlag={0} and a.IsApp=1", aast);
                }
            }
            var sqlorderby = string.Format(" order by b.{0} ", jsRenderSortField);
            //List<PrincipleInfor> list = Approval.GetMyPrinciplePaging(out totalCount, sqlwhere, pageIndex, pageSize, sqlorderby);
            List<PrincipleInfor> list = Approval.GetMyPrinciplePagingAllType(out totalCount, sqlwhere, pageIndex, pageSize, sqlorderby);
        ...
        public JsonResult GetPrincipleManageList(string courseName = "", string realName = "", string grade = "-1", int status = -1, int attstatus = -1, string startTime = "2000-1-1", string endTime = "2050-1-1", int pageIndex = 1, int pageSize = 20, string jsRenderSortField = "a.AppDateTime desc")
        {
            ...
            var sqlwhere = string.Format("{0} {1} {2} {3} {4} {5} {6} {7}",
                                         courseName.Trim() == ""
                                             ? ""
                                             : " and b.CourseName like '%" + CodeHelper.ReplaceSql(courseName.Trim()) + "%' ",
                                         " and a.ApprovalUser='" + CurrentUser.JobNum + "' ",
            ...
            list = Approval.GetMyPrinciplePagingAllType(out totalCount, sqlwhere, pageIndex, pageSize, sqlorderby);
        ...
        public JsonResult GetPrincipleTrainManageList(string courseName = "", string realName = "", string dpname = "", string grade = "-1", int aat = -1, int status = -1, int attstatus = -1, int pageIndex = 1, int pageSize = 20, string jsRenderSortField = "b.StartTime desc")
        {
            ...
            switch (attstatus)
            {
                ...
                (
                   (b.AttFlag=3 and a.StartTime>b.StartTime and a.EndTime>=b.EndTime) or
                   (b.AttFlag=1 and a.StartTime>b.StartTime)
                ) and a.LessLength<=0 AND  a.StartTime<'2050/1/1 0:00:00' ";
                    break;
                case 1:
                    aststr += @" and
                (
            ...
            if (status != -1)
            {
                ...
                {
                    sqlwhere += string.Format(" and a.ApprovalFlag={0} and a.IsApp=1", status);
                }
            }
        ...
        public ActionResult MyApprovalIndex()
        {
            return View();
        }
        ...
        /// <returns></returns>
        [Filter.SystemLog("审批违纪申辩", LogLevel.Info)]
        public JsonResult ApprovalPrinApp(int id, string memo, int result, int userID, int courseID, decimal lesslength)
        {
            ...
            Cl_Attendce attmodel = IAttendce.GetCl_Attendce(courseID, userID); //缺勤添加了AttStatus=1是缺勤(缺勤审批后都获得0) 0则是迟到,早退,迟到或早退
            if (Approval.ExplainPrin(str) > 0)
            {
                ...
                if (corder != null)
                {
                    ...
                    if ((corder.OrderStatus == 3 && corder.MakeUpApprovalFlag != 2 && result == 1) || (corder.OrderStatus != 3 && result == 1))
                    {
                        ...
                        {
                            Approval.ChangeCourseOrderInfor(string.Format("update Cl_CourseOrder set GetScore={0},AttScore={3} where UserId={1} and CourseId={2} ", 0, userID, courseID, 0));
                        }
                    }
                }
        ...
        #region  违纪方法
        public JsonResult GetMyApprovalPrincipleListForWeiJi(string courseName = "", int ast = -1, int aat = -1, int aast = -1, string startTime = "2000-1-1", string endTime = "2050-1-1", int pageIndex = 1, int pageSize = 20, string jsRenderSortField = "StartTime desc")
        {
            string sql = " 1=1";
            #region 查询条件
            ...
            if (Convert.ToDateTime(startTime) != Convert.ToDateTime("2000-1-1") && Convert.ToDateTime(endTime) != Convert.ToDateTime("2050-1-1"))
            {
                str = string.Format(" and (Co_Course.StartTime between '{0}' and '{1}') ", Convert.ToDateTime(startTime).ToString("yyyy-MM-dd HH:mm:ss"), Convert.ToDateTime(endTime).AddDays(1).AddMilliseconds(-1));
            }
            else if (Convert.ToDateTime(startTime) != Convert.ToDateTime("2000-1-1") && Convert.ToDateTime(endTime) == Convert.ToDateTime("2050-1-1"))
            {
                str = string.Format(" and Co_Course.StartTime >= '{0}' ", Convert.ToDateTime(startTime).ToString("yyyy-MM-dd HH:mm:ss"));
            }
            else if (Convert.ToDateTime(startTime) == Convert.ToDateTime("2000-1-1") && Convert.ToDateTime(endTime) != Convert.ToDateTime("2050-1-1"))
            {
                str = string.Format(" and Co_Course.StartTime <= '{0}' ", Convert.ToDateTime(endTime).AddDays(1).AddMilliseconds(-1));
            }
            ...
            {
                sql += string.Format(" AND Co_Course.StartTime>='{0}'", startTime);
            }
            if (!string.IsNullOrEmpty(endTime))
            {
                sql += string.Format(" AND Co_Course.EndTime<='{0}'", Convert.ToDateTime(endTime).AddDays(1).AddSeconds(-1));
            }
            var sqlwhere = string.Format("{0} {1} {2} {3} {4} {5} ",
                                          courseName.Trim() == ""
                                              ? ""
                                              : " and Co_Course.CourseName like '%" + CodeHelper.ReplaceSql(courseName.Trim()) + "%' ",
                //" and Cl_Attendce.UserId='" + CurrentUser.UserId + "' ",
                                          "",
                                          str,
                                          sql,
                                          aat == -1 ? "" : (string.Format(" and Cl_Attendce.IsApp={0}", aat)),
                                          aast == -1 ? "" : (string.Format(" and Cl_Attendce.ApprovalFlag={0}", aast))
                                          );
            var sqlorderby = string.Format(" order by Co_Course.{0} ", jsRenderSortField);
            #endregion
            int count = 0;
            var list = IAttendce.GetMyAttendcsList(out count, CurrentUser.UserId, sqlwhere, (pageIndex - 1) * pageSize + 1, pageSize);
