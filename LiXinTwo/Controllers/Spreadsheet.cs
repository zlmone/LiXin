...
namespace LiXinControllers
{
    public class Spreadsheet : Controller
    {
        ...
        /// <returns></returns>
        public List<DataTable> LoadExcel(string path)
        {
            DataSet dataset = RenderFromExcel(path);
        ...
        public IWorkbook DrawChart(List<ChartModel> columns, List<DataTable> allitem, string sheetname, bool OpenSheet = false)
        {
            //创建新的workbook
            ...
            for (int a = 0; a < columns.Count; a++)
            {
                ...
                IChart chart = shapes.AddChart(chartleft, charttop, chartright - chartleft, chartbottom - charttop).Chart;
        ...
        public IWorkbook ExportExcel_One(string title, string subtitle, DataTable allitem, string sheetname)
        {
            //创建新的workbook
            IWorkbook workbook = Factory.GetWorkbook();
        ...
        /// <returns></returns>
        public IWorkbook BuildSheet(List<SheetModel> sheets)
        {
            //创建新的workbook
            IWorkbook workbook = Factory.GetWorkbook();
            IWorksheets worksheets = workbook.Worksheets;
                    ...
                    if (dataList[j].datatype.ToString() != "DataTable")
                    {
                        ...
                        switch (dataList[j].datatype.ToString())
                        {
                            ...
                            case "Pie":
                                ...
                                seriesTotal.HasDataLabels = true;
                                seriesTotal.DataLabels.ShowPercentage = true;
                                seriesTotal.DataLabels.ShowValue = true;
                                seriesTotal.DataLabels.NumberFormat = "0.00%";
                                break;
                        }
        ...
        public void Downfile(IWorkbook workbook, HttpContextBase context, string excelName = "report")
        {
        ...
        /// <summary>
        /// 直接将html导出成excel,会出现格式错误的提示
        /// </summary>
        /// <param name="ExcelHtml"></param>
        public void ExportExcel_Html(string ExcelHtml, string title, HttpContextBase context)
        {
            #region 表头
            StringBuilder htmlbody = new StringBuilder(@"<html xmlns:v='urn:schemas-microsoft-com:vml' xmlns:o='urn:schemas-microsoft-com:office:office' xmlns:x='urn:schemas-microsoft-com:office:excel' xmlns='http://www.w3.org/TR/REC-html40'>
                                <head>
...
<style type='text/css'>
        ...
        .gdtjContainer .tb tr th.rightborder
        {
            ...
            //filenames是自定义的文件名
            context.Response.AppendHeader("Content-Disposition", "attachment;filename=" + HttpUtility.UrlEncode(name, Encoding.UTF8).ToString() + ".xls");
            //content是步骤1的html，注意是string类型
            htmlbody.AppendFormat(ExcelHtml + "</body></html>");
            context.Response.Write(htmlbody.ToString());
            context.Response.End();

        }

      
        public DataSet RenderFromExcel(string path)
        {
            DataSet dataSet = new DataSet();
            IWorkbook workbook = Factory.GetWorkbook(path, System.Globalization.CultureInfo.CurrentCulture);
            dataSet = workbook.GetDataSet(GetDataFlags.FormattedText);
            return dataSet;
        }
        ...
        /// <returns></returns>
        public static DataModel ChangChartModelToDataModel(ChartModel chartitem)
        {
