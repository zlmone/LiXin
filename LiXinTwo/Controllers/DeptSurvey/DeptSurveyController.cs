...
namespace LiXinControllers.DeptSurvey
{
    ...
    {
        protected IDeptSurveyExampaper SeBl;
        protected IDepSurvey_Survey surveyBL;
        protected IDepSurveyReplyAnswer replayAnswerBL;
        protected IDepartment deptBL;
        protected ISys_Group sys_GroupBL;
        protected ISys_TrainGrade trainBL;
        public DeptSurveyController(IDeptSurveyExampaper _SeBl, IDepSurvey_Survey _surveyBL, IDepSurveyReplyAnswer _replayAnswerBL, IDepartment _deptBL, ISys_Group _sys_GroupBL, ISys_TrainGrade _trainBL)
        {
            SeBl = _SeBl;
            surveyBL = _surveyBL;
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult DeptSurveyExampaperList()
        {
            ViewBag.DepartList = GetAllSubDepartments();//deptBL.GetAllList(" DepartmentId IN ( SELECT ID FROM dbo.f_GetDeptChildByDeptID(" + CurrentUser.DeptId + "))");
            return View();
        }
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult DeptSortEdit()
        {
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult DeptSurveyExampaperEdit(int id, int sortID, int DeptID)
        {
        ...
        public ActionResult DeptAddSurveyQuestion()
        {
            return View();
        }

        public ActionResult DeptImportQuestion()
        {
            return View();
        }
        ...
        /// <returns></returns>
        public ActionResult DeptBrowsePaper(int id)
        {
            var examPaper = SeBl.GetSurveyExampaper(id);
            return View(examPaper);
        }

        public ActionResult DeptCourseChooseSurvey(int DeptID)
        {
            ViewBag.DeptID = DeptID;
            return View();
        }
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult GetAllPaperSortTree(int DeptID)
        {
            var where = string.Format(" IsDelete=0 and DeptID ={0}", DeptID);
            var paperSort = SeBl.GetPaperSortList(where).ToList();
        ...
        /// <returns></returns>
        public JsonResult IsExsitPaperSort(int id,int DeptID, int pId, string name)
        {
            try
            {
                var where = string.Format(" DeptID={0}", DeptID);
                return Json(SeBl.IsExsitPaperSort(name, id, pId, where), JsonRequestBehavior.AllowGet);
            }
            catch (Exception)
            {
        ...
        /// </summary>
        /// <returns></returns>
        public JsonResult SubmitPaperSort(int DeptID)
        {

            var id = Request.QueryString["id"].StringToInt32();
            var pId = Request.Form["hidfatherID"].StringToInt32();
            var where = string.Format(" DeptID={0}", DeptID);
            if (SeBl.IsExsitPaperSort(Request.Form["txtSortName"].Trim(), id, pId,where))
            {
                if (id == 0)
                {
                    //添加
        ...
        public JsonResult DeletePaperSortByID(int id)
        {
            if (id > 0)
            {
                var list = SeBl.GetServeyExamPaperList(string.Format(" Dep_Survey_Exampaper.IsDelete=0 and SortID={0} ", id));
        ...
        /// <returns></returns>
        public JsonResult GetAllPaper(int sortID = 0, string name = "", int type = -1,int DeptID=0, int pageIndex = 1, int pageSize = 20, string jsRenderSortField = " LastUpdateTime desc")
        {
            ...
            int totalCount;
            var paperList = SeBl.GetServeyExamPaperListPaging(out totalCount, string.Format(" Dep_Survey_Exampaper.SortID in ({0}) and Dep_Survey_Exampaper.IsDelete=0 and Dep_Survey_Exampaper.ExamTitle like '%{1}%' and {2} and  Dep_Survey_Exampaper.DeptID={3}", sortstr, name.ReplaceSql(), (type == -1 ? "1=1" : ("Dep_Survey_Exampaper.ExamType=" + type)), DeptID), pageIndex, pageSize, jsRenderSortField);
        ...
        /// </summary>
        /// <returns></returns>
         [Filter.SystemLog("保存问卷", LogLevel.Info)]
