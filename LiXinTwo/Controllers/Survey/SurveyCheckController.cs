...
namespace LiXinControllers
{
    ...
    {
        // public static List<Survey_user> ListSurveyUser = new List<Survey_user>(); 
        #region 呈现
        ...
        public ActionResult SurveyCheckDeatilsForOffice(int userNum, int surveyID, int paperID, int shouldNumber = 0)
        {
            var survey = surveyBL.Get(surveyID);
            survey.useNum = userNum;
            ViewBag.paperID = paperID;
            ViewBag.surveyID = surveyID;
            survey.ShouldNumber = shouldNumber;
            return View(survey);
        }

        public ActionResult studentSubmit(int surveyID, int paperID)
        {
            var totalcount = 0;
            int noanswerCount = surveyBL.GetUserNoJoin(out totalcount, surveyID).Count();
            int answerCount = surveyBL.GetUserAnswer(out totalcount, surveyID).Count();
            ViewBag.noanswerCount = noanswerCount;
            ViewBag.sumCount = noanswerCount + answerCount;
        ...
        /// </summary>
        /// <returns></returns>
        public JsonResult GetStudentSubmit(int type, int surveyID, string realName = "", string deptName = "", int ISCPA = 99, int Sex = -1, string TrainGrade = "", int pageSize = 10, int pageIndex = 1, string jsRenderSortField = " SubmitTime desc")
        {
            try
            {
                var where = " 1=1";
                ...
                if (TrainGrade != "")
                {
                    where += " and TrainGrade in (select Id from dbo.F_SplitIDs('" + TrainGrade + "'))";
                }
                var ListSurveyUser = new List<Survey_user>();
                var totalcount = 0;
                if (type == 0)
                {
                    ListSurveyUser = surveyBL.GetUserAnswer(out totalcount, surveyID, pageIndex, pageSize, where, jsRenderSortField);
                }
                else
                {
                    jsRenderSortField = jsRenderSortField.Trim() == "SubmitTime DESC" ? "userId desc" : jsRenderSortField;
                    ListSurveyUser = surveyBL.GetUserNoJoin(out totalcount, surveyID, pageIndex, pageSize, where, jsRenderSortField);
                }
        ...
        /// 导出EXCEl
        /// </summary>
        public void OutData(int type, int surveyID, string realName = "", string deptName = "", int ISCPA = 99, int Sex = -1, string TrainGrade = "")
        {
            var where = " 1=1";
            if (realName != "")
            {
            ...
            if (TrainGrade != "")
            {
                where += " TrainGrade in (select Id from dbo.F_SplitIDs(" + TrainGrade + "))";
            }
            var ListSurveyUser = new List<Survey_user>();
            var totalcount = 0;
            if (type == 0)
            {
                ListSurveyUser = surveyBL.GetUserAnswer(out totalcount, surveyID, where: where, jsRenderSortField: "StartTime desc");
            }
            else
            {

                ListSurveyUser = surveyBL.GetUserNoJoin(out totalcount, surveyID, where: where, jsRenderSortField: "UserId desc");
            }
            ...
            if (type == 0)
            {
                outTable.Columns.Add("提交时间", typeof(string));
            }
            outTable.Columns.Add("性别", typeof(string));
            outTable.Columns.Add("部门/分所", typeof(string));
            outTable.Columns.Add("培训级别", typeof(string));
            outTable.Columns.Add("是否CPA", typeof(string));
            var index = 1;
            foreach (var v in ListSurveyUser)
            {
                if (type == 0)
                {
                    outTable.Rows.Add(index, v.Realname, v.SubmitTimeStr, v.SexStr, v.DeptPath, v.TrainGrade, v.CPA);
                }
                else
                {
                    outTable.Rows.Add(index, v.Realname, v.SexStr, v.DeptPath, v.TrainGrade, v.CPA);
                }
                //  outTable.Rows.Add(index, v.CourseName, v.LearnNumber, v.ReallyNumber, v.JoinRate, v.PassRate, v.Survey_AllScore, v.Survey_CourseScore, v.Survey_TeacherScore);
                index++;
            }
            var typetext = type == 0 ? "已提交" : "未提交";
            new Spreadsheet().Template("员工提交明细 " + typetext, null, outTable, HttpContext, "员工提交明细 " + typetext, "员工提交明细 " + typetext);
        }
        #endregion

        #region 部门树的方法
        public ActionResult DeptTree(int surveyID, int paperID, int type = 0)
        {
            int depID = 0;
            var treeStr = GetDeptTreeStr(out depID, surveyID, paperID, type);
            return Json(new
            {
                depID = depID,
        ...
        public StringBuilder GetDeptTreeStr(out int depID, int surveyID, int paperID, int type)
        {
            string prefix = "";
