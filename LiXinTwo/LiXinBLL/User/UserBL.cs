...
namespace LiXinBLL.User
{
    public class UserBL : IUser
    {
        ...
        /// <returns></returns>
        public decimal GetUserScore(int uid, int year, Sys_ParamConfig cpazq, bool flag = true)
        {
        ...
        {
            var usermodel = _userDB.GetmodelCAP(uid, year);
            usermodel.CoScore = TogetherSum;
            usermodel.voScore = VideoSum;
            usermodel.CPAToScore = CPASum;
            usermodel.DepScore = DepSum;
            return usermodel;
        }
        /// <summary>
        /// 学分前5名
        ...
        public List<Co_CourseShow> GetMyCourse(int userid, int year, int way, out int totalcount, int startIndex = 0, int startLength = int.MaxValue, string Order = "asc", string where = "1=1")
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                if (list[i].CoPaperID > 0 && list[i].PaperId > 0)
                {
                    ...
                    list[i].ExamScore = paper == null ? 0 : paper.ExampaperScore;
                }
                else
                {
        ...
        {
            var list = _userDB.GetMyCPACourse(userid, year,Order,where);
            //totalcount = list.Count > 0 ? list[0].totalcount : 0;
            return list;
        }
        ...
        {
            var list = _userDB.GetMyCPACourse(userid, bentime, endtime,Order, where);
            //totalcount = list.Count > 0 ? list[0].totalcount : 0;
            return list;
        }
        ...
        {
            return _userDB.GetLeardForUserID(userID);
        }
        #endregion
		...
		//{
		//	var list = _userDB.GetCPACourseShow(userid,year,Order,where);
