...
namespace LiXinBLL.AttendceManage
{
    public class AttendceBL : IAttendce
    {
        ...
        {
            List<Cl_Attendce> tempcu = AttDB.GetAttendUserList(CourseId, Order, where,overwhere);
            return tempcu;
        }
        ...
        public string AddAttendces(int CourseId, DataTable dtlist)
        {
            ...
            return "成功" + cg + "条，失败" + sb  + "条,错误信息为"+error;
        }
        ...
        public void AddAttendce(int CourseId, int userid, string starttime, string endtime, int AttStatus=0)
        {
            //Co_Course coUser = AttDB.GetCoAndUser(CourseId, userid);
            Sys_User leamodel = GetLeaderIdByUserId(userid);
            Cl_Attendce attmodel = clAttDB.GetCl_Attendce(CourseId, userid);
        ...
        /// <returns></returns>
        public bool Cl_MakeUpOrderCount(int count, int userid,int year=0)
        {
            ...
            {
                where = string.Format("  UserId={0} AND YearPlan={1}", userid, year);
            }
