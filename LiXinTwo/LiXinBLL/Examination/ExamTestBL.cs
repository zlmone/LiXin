...
namespace LiXinBLL.Examination
{
    public class ExamTestBL : IExamTest
    {
        ...
        public ExamTestBL(ICo_CoursePaper _ICoCoursePaperBL, ICo_Course _ICourseBL, ISys_ParamConfig _ISysParamConfigBL,
            INew_Course _inewcourseBL, INew_CoursePaper _inewcoursepaperBL, IDep_Course _Idep_courseBL, IDep_CoursePaper _Idep_coursepaperBL)
        {
            Etdb = new ExamTestDB();
        ...
        /// <param name="coursePaperId"> </param>
        ...
        {
            return new ExaminationDB().GetExamSendStudentBySQL2008(relationId, userId, coursePaperId, examPaperID,
                                                                   sourceType);
        }
        ...
        public ExamBaseInforShow GetExamBaseInforShow(int examUserID, int type = 0)
        {
            ...
            else if (type == 3)
            {
                if (examuser != null)
                {
                    var coursepaper = inewcoursepaperBL.GetSingleCoursePaper(examuser.RelationID);
                    examBaseInfor.ExamLength = coursepaper.Length;

                    var course = inewcourseBL.GetSingleCourse(examuser.RelationID);

                    examBaseInfor.ExamStartTime = course.EndTimeStr;
                    examBaseInfor.ExamEndTime = course.EndTime.AddHours(Convert.ToDouble(coursepaper.Hour)).ToString("yyyy-MM-dd HH:mm"); ;
                    //var exam = Etdb.GetSingleByID<tbExamination>(examuser.RelationID);
                    var paper = Etdb.GetSingleByID<tbExampaper>(examuser.ExamPaperID);
                    examBaseInfor.PassScore = Convert.ToInt32(coursepaper.LevelScore * 0.01 * paper.ExampaperScore);//考试通过分数线
                    if (paper.ExamType == 0)
            ...
            else if (type == 5)
            {
                if (examuser != null)
                {
                    var coursepaper = Idep_coursepaperBL.GetCo_CourseMainPaper(examuser.RelationID);
                    examBaseInfor.ExamLength = coursepaper.Length;

                    var course = Idep_courseBL.GetCo_Course(examuser.RelationID);
        ...
        /// </summary>
        /// <param name="examUserID">考试人员ID</param>
        /// <param name="type">0:独立的考试 1：课程里面的考试</param>
        /// <returns></returns>
            ...
            //{
                ...
                //{
                    exampaper.QuestionList
                              //.Where(p => p.QType == s.StringToInt32())
                             .ToList()
                             .RandomListOrder()
                             .ForEach(p =>
                             {
                ...
                foreach (var item in exampaper.QuestionList)
                {
                    ...
                    item.QuestionAnswer = item.QuestionAnswer.ToList()
                         .RandomListOrder();

                    int answerOrder = 1;
                    foreach (var model in item.QuestionAnswer)
                    {
                        model.AnswerOrder = answerOrder;
                        answerOrder++;
                    }
