...
namespace LiXinExam.Controllers
{
    public partial class QuestionController
    {
        ...
        ///     问题编辑页面呈现
        /// </summary>
        public ActionResult DepQuestionEdit(int id)
        {
            ViewBag.Flag = 1;//部门分所试题编辑
            Response.Expires = 0;
            if (id > 0)
            {
                var qu = EQuestionBL.GetSingleByID(id);
        ...
        /// <returns></returns>
        [ValidateInput(false)]
        [LiXinControllers.Filter.SystemLog("编辑问题", LogLevel.Info)]
        public JsonResult SubmitQuestion(int id, int type, int deptId)
        {
            var content = Request.Form["hiddenQuestionContent"].NoHtml();
            ...
            //类型
            var qType = Convert.ToInt32(Request.Form["hiddenSelQuestionType"]);
            var newid = 0;
            var flag = true;
            if (qType == 6)
            {
                ...
                {
                    if (((type == 0 || files[i].ContentLength <= 8388608) &&
                         (type != 0 || files[i].ContentLength <= 512000)) || !flag) continue;
                    flag = false;
                    break;
                }
            }
            var questionType = Convert.ToInt32(Request.Form["hiddenSelQuestionType"]);

            if (flag)
            {
                var listfileUpload = qType == 6 ? FileUpload(type) : new List<FileUpload>();
                var question = id > 0
