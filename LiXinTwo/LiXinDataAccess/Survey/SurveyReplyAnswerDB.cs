...
namespace LiXinDataAccess.Survey
{
    public class SurveyReplyAnswerDB : BaseRepository
    {
        public Survey_ReplyAnswer GetSurvey_QuestionByQuestionID(int AnswerId)
        {
            ...
            {
                const string sqlstr = "select * from Survey_ReplyAnswer where AnswerId=@AnswerId";
        ...
        public bool InsertSurvey_ReplyAnswer(Survey_ReplyAnswer model)
        {
            using (var conn = OpenConnection())
            {
                const string sqlwhere =
                    @"INSERT INTO dbo.Survey_ReplyAnswer
	                (
	                SourceFrom,
	                ObjectID,
	                ExampaperID,
        ...
        public List<Survey_ReplyAnswer> GetList(string where = " 1=1")
        {
            using (var connection = OpenConnection())
            {
                string sqlstr = "select * from Survey_ReplyAnswer where " + where;
        ...
        public List<Survey_ReplyAnswer> GetSurvey_ReplyAnswerListAndUser(string where = " 1=1")
        {
            using (var connection = OpenConnection())
            {
                string sqlstr = "  select Sys_User.Realname,Sys_User.DeptName,* from Survey_ReplyAnswer left join Sys_user on Survey_ReplyAnswer.UserId=Sys_User.UserId where " + where;
                return connection.Query<Survey_ReplyAnswer>(sqlstr).ToList();
            }
        }
        ...
        public bool GetStatus(int CourseId, int UserId, int status, int ExampaperID)
        {
            using (var connection = OpenConnection())
            {
                string sql = "select count(1) from Survey_ReplyAnswer where UserID=" + UserId + " and ObjectID=" +
        ...
        public bool UpdateStatus(int CourseId, int UserId, int status, int ExampaperID)
        {
            using (var connection = OpenConnection())
            {
                string sql = " update Survey_ReplyAnswer set [Status]=" + status + " where  UserID=" + UserId + " and ObjectID=" + CourseId + " and ExampaperID=" + ExampaperID;
                int result = connection.Query<int>(sql).FirstOrDefault();
                return result > 0;
            }
        }
        ...
        public int GetSurvey_ReplyAnswerCountBYbjectID(int courseid, int ExampaperID, int QuestionId)
        {
            using (var connection = OpenConnection())
            {
                string sql = "select count(1) from Survey_ReplyAnswer where  ObjectID=" + courseid + " and ExampaperID=" +
        ...
        public int GetSingleSurvey_ReplyAnswerCount(int courseid, int ExampaperID, int QuestionId, int AnswerId, int UserId)
        {
            using (var connection = OpenConnection())
            {
                ...
                if (UserId == 0)
                {
                    sql = "  select count(1) from Survey_ReplyAnswer where  ObjectID=" + courseid + " and ExampaperID=" + ExampaperID + " and ObjectiveAnswer  like '%" + AnswerId + "%' and QuestionId=" + QuestionId;
                }
                else
                {
                    sql = "  select count(1) from Survey_ReplyAnswer where  ObjectID=" + courseid + " and ExampaperID=" + ExampaperID + " and ObjectiveAnswer like '%" + AnswerId + "%' and QuestionId=" + QuestionId + " and UserID=" + UserId;
                }
                int result = connection.Query<int>(sql).FirstOrDefault();
                return result;
            }
        ...
        public int GetCount(int CourseId, int UserId)
        {
            using (var connection = OpenConnection())
            {
                ...
                sql += " select* from ";
                sql += " Co_Course left ";
                sql += " join Survey_ReplyAnswer ";
                sql += " on";
                sql += " Co_Course.Id = Survey_ReplyAnswer.ObjectID";
                sql += " where";
                sql += " Survey_ReplyAnswer.SourceFrom = 0";
                sql += " and";
                sql += " Survey_ReplyAnswer.UserID = " + UserId + "";
                sql += " and";
                sql += " Survey_ReplyAnswer.ObjectID = " + CourseId + "";
        ...
        public bool DeleteByCourseIDAndUserId(int CourseID, int UserId, int ExampaperID)
        {
            using (var connection = OpenConnection())
            {
                string sql = " delete Survey_ReplyAnswer where ObjectID=" + CourseID + " and UserID=" + UserId + " and ExampaperID=" + ExampaperID + " and SourceFrom=0 and [Status]=0";
        ...
        public void UpdateReaplyAnswer(Survey_ReplyAnswer model)
        {
            using (var conn = OpenConnection())
            {
                const string sql = @"UPDATE dbo.Survey_ReplyAnswer
                SET 
