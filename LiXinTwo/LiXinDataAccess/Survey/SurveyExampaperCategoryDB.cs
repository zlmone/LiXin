...
and IsDelete=0";
        ...
        /// <param name="sortName">问卷分类名称</param>
        /// <param name="parentId">父级ID</param>
        public int GetPaperSortByName(string sortName,int parentId)
        {
            using (var conn = OpenConnection())
            {
                const string sqlwhere =
                    @"select CategoryId from Survey_ExampaperCategory where CategoryName=@CategoryName and ParentId=@ParentId
and IsDelete=0 ";
                var param = new
                {
                ...
                var categoryId = conn.Query<int>(sqlwhere, param).FirstOrDefault();
                return categoryId;
            }
        }
    }
