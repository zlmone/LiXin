...
{
    public class DeptCoursePaperDB : BaseRepository
    {
        /// <summary>
        /// 新增课程关联考试
        /// </summary>
...
values (@PaperId,@CourseId,@Length,@Hour,@LevelScore,@TestTimes,@IsMain)";
        ...
        /// </summary>
        /// <param name="model"></param>
        public bool UpdateCourseMainPaper(Dept_CoursePaper model)
        {
            using (var conn = OpenConnection())
            {
                const string sqlwhere =
        ...
        public Dept_CoursePaper GetCo_CourseMainPaper(int CourseId)
        {
            using (var conn = OpenConnection())
            {
                ...
                return conn.Query<Dept_CoursePaper>(sql).FirstOrDefault();
            }
        }
