...
namespace LiXinDataAccess.AttendceManage
{
    public class AttendceDB : BaseRepository
    {
        ...
        /// 获得考勤人员数据列表
        /// </summary>
        public List<Cl_Attendce> GetAttendUserList(int CourseId,string Order = "asc", string where = "1=1",string overwhere=" 1=1")
        {
            using (IDbConnection conn = OpenConnection())
            {
                //2013-10-12 毛佳源 添加列 su.TrainGrade,su.CPANo,CASE WHEN (dbo.F_IsAppStatus(cc.Id,cl.UserId)=1) then cl.GetScore  ELSE 0 END as GetScore,cc.CourseLengthDistribute,cc.CourseLength
                string sql =string.Format(@" SELECT * FROM(
SELECT cc.NumberLimited,cl.UserId,cl.CourseId,cl.IsLeave,cl.LearnStatus,cl.ApprovalFlag,
cl.ApprovalDateTime,cc.StartTime AS courseStart,cc.EndTime AS courseEnd,
cc.AttFlag,cc.AttStatus,cl.ApprovalLimitTime,cl.OrderTime,cl.OrderEndTime,
ca.StartTime,ca.EndTime,su.Realname,su.CPA,sd.DeptName,su.TrainGrade,
su.CPANo,
CASE WHEN (dbo.F_NewIsAppStatus(cc.Id,cl.UserId,cl.OrderStatus)=1) then cl.GetScore  ELSE 0 END as GetScore,
cc.CourseLengthDistribute,cc.CourseLength,cl.OrderStatus,isnull(cm.Id,0) AS MakeOrederId ,
orderOverTime=(SELECT count(*) FROM View_OrderTimes WHERE UserId=su.UserId)
FROM Cl_CourseOrder AS cl left JOIN Sys_User AS su ON su.UserId=cl.UserId 
LEFT JOIN Sys_Department AS sd ON su.DeptId=sd.DepartmentId 
LEFT JOIN  Cl_Attendce AS ca ON (ca.UserId=cl.UserId AND cl.CourseId=ca.CourseId) LEFT JOIN Cl_MakeUpOrder AS cm ON (cm.UserId=cl.UserId AND cl.CourseId=cm.CourseId) LEFT JOIN Co_Course AS cc ON cc.Id=cl.CourseId where cl.Id NOT IN  (SELECT cco.Id FROM Cl_CourseOrder AS cco LEFT JOIN Cl_TimeOutLeave AS cto ON(cco.CourseId=cto.CourseId AND cco.UserId=cto.UserId) where ((cco.ApprovalFlag=1 AND cco.ApprovalDateTime<=cco.ApprovalLimitTime and cco.IsLeave=1 AND cco.OrderStatus=1 ) OR cco.OrderStatus IN (0,2) OR (cco.OrderStatus=1 AND cto.ApprovalFlag=1)) 
AND cco.CourseId=@CourseId) AND cl.CourseId=@CourseId and {0}
)t where {1}", where, overwhere);
                var param = new
                {
                    CourseId = CourseId
                };
...
WHERE su.Realname='" + username + "' AND sd.DeptName='" + deptname + "'";
        ...
        public Co_Course GetCoAndUser(int courseid, int userid)
        {
            using (IDbConnection conn = OpenConnection())
            {
                string sql = @"SELECT *,(SELECT LeaderID FROM Sys_User WHERE UserId=@UserId) AS LeaderID FROM Co_Course WHERE Id=@CourseId";
                var param = new
                {
                    UserId = userid,
                    CourseId = courseid
        ...
        public bool ExistAtts(int CourseId, int UserId)
        {
            using (IDbConnection conn = OpenConnection())
            {
                string sqlwhere = @"select count(1) FROM Cl_Attendce WHERE  AttStatus=0 and  CourseId IN (SELECT cc.id FROM dbo.Co_Course AS cc 
LEFT JOIN Co_Course AS ccc ON cc.Code=ccc.Code 
WHERE cc.IsDelete=0 AND cc.Publishflag=1 AND ccc.Id=@CourseId) AND UserId=@UserId AND CourseId!=@CourseId";
