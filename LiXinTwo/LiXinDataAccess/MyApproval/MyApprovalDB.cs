...
where rowNum between  @pageCount*@pageIndex+1 and @pageCount*(@pageIndex+1)", pageLength, orderBy);
        ...
        /// <summary>
        /// 获取需要我审批的课程违纪记录 2013-10-22 新加 毛佳源
        /// </summary>
        ...
        /// <returns></returns>
        public List<PrincipleInfor> GetMyApprovalPrinciplePagingForNew(out int totalCount, string where = " and 1 = 1 ", int startIndex = 0, int pageLength = int.MaxValue, string orderBy = " order by a.AppDateTime desc ")
        {
            using (var connection = OpenConnection())
            {
                totalCount = connection.Query<int>("select count(1) from Cl_Attendce as a,Co_Course as b,Sys_User as c where a.CourseId=b.Id and a.UserId=c.UserId " +
                    "and (select count(*) from cl_courseorder where userid=a.userid and courseid=a.courseid and  (	(b.AttFlag=1 and a.StartTime>b.StartTime) or (b.AttFlag=2 and a.EndTime<b.EndTime)	or	(b.AttFlag=3 and ((a.StartTime>=b.StartTime and a.EndTime<=b.EndTime)) or a.StartTime>=b.StartTime or a.EndTime<=b.EndTime)  ) and (isleave=0 or (isleave=1 and approvalFlag<>1) or (isleave=1 and approvalFlag=1 and approvaldatetime>ApprovalLimitTime)))>0  and a.IsApp=1" +
                                                   "" + where).First();//and a.LessLength<0 
...
where a.CourseId=c.Id and a.UserId=b.UserId and a.Id=@id");
        ...
        /// <summary>
        /// 获取我的违纪情况列表  所有状态：迟到-早退-缺勤-迟到且早退
        /// </summary>
        /// <param name="totalCount">总数</param>
        /// <param name="where">查询条件</param>
        /// <param name="startIndex">开始页数</param>
        ...
        /// <param name="orderBy">排序条件</param>
        /// <returns></returns>
        public List<PrincipleInfor> GetMyPrinciplePagingAllType(out int totalCount, string where = " and 1 = 1 ", int startIndex = 0, int pageLength = int.MaxValue, string orderBy = " order by b.StartTime desc ")
        {
            using (var connection = OpenConnection())
            {
                totalCount = connection.Query<int>("select count(1) from Cl_Attendce as a,Co_Course as b,Sys_User as c where a.CourseId=b.Id and a.UserId=c.UserId " +
                    "and (select count(*) from cl_courseorder where userid=a.userid and courseid=a.courseid and  (	(b.AttFlag=1 and a.StartTime>b.StartTime) or (b.AttFlag=2 and a.EndTime<b.EndTime)	or	(b.AttFlag=3 and ((a.StartTime>=b.StartTime and a.EndTime<=b.EndTime) or a.StartTime>=b.StartTime or a.EndTime<=b.EndTime))) and (isleave=0 or (isleave=1 and approvalFlag<>1) or (isleave=1 and approvalFlag=1 and approvaldatetime>ApprovalLimitTime)))>0 " +
                                                   "" + where).First();//and a.LessLength<0 
                var query = string.Format(@"
select top {0} * from (
    select  row_number() over( {1} ) as rowNum,a.Id as ID,a.UserId as UserID,c.Realname as RealName,c.DeptName as DepartName,
...
and 
  (
	...
	(
		b.AttFlag=3 and (
						(a.StartTime>=dateadd(n,1,b.StartTime) and a.EndTime<=dateadd(n,1,b.EndTime))
						or
						a.StartTime>=dateadd(n,1,b.StartTime)
						or
						a.EndTime<=dateadd(n,1,b.EndTime)
						)
	)
  )
--and a.LessLength<0
...
where a.CourseId=c.Id and a.UserId=b.UserId and a.Id=@id");
        ...
        public List<PrincipleInfor> GetPrincipleTrainManagePaging(out int totalCount, string where = " and 1 = 1 ", int startIndex = 0, int pageLength = int.MaxValue, string orderBy = " order by b.StartTime desc ")
        {
            using (var connection = OpenConnection())
            {
                totalCount = connection.Query<int>("select count(1) from Cl_Attendce as a,Co_Course as b,Sys_User as c where a.CourseId=b.Id and a.UserId=c.UserId  " + where).First(); //and a.LessLength<0 
                var query = string.Format(@"
select top {0} * from (
    select  row_number() over( {1} ) as rowNum,a.Id as ID,a.UserId as UserID,c.Realname as RealName,c.DeptName as DepartName,
...
WHERE OrderStatus=1 AND cco.IsLeave=1 AND ((cco.ApprovalFlag=0 AND cco.ApprovalLimitTime<getdate()) or (cco.ApprovalFlag>0 AND cco.ApprovalDateTime>cco.ApprovalLimitTime))
) result
where  {1}
