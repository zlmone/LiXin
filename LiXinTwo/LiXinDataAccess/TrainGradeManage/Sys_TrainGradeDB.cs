...
namespace LiXinDataAccess
{
    public class Sys_TrainGradeDB : BaseRepository
    {
        ...
        /// <returns></returns>
        public List<Sys_TrainGrade> GetUserTrain(int year, int userID, string where = "1=1", int startIndex = 1,
            int startLength = int.MaxValue, string ordersql = "ORDER BY su.DeptPath asc,su.TrainGrade asc", string where1="1=1")
        {
...
SELECT * from (
SELECT row_number()OVER(ORDER BY DeptPath asc,TrainGrade asc) number,* FROM (
...
WHERE number BETWEEN @startLength*(@startIndex-1)+1  AND @startLength*@startIndex", where, ordersql, where1);
                var param = new
               {
...
WHERE IsDelete=0 AND IsTeacher<2 AND PayGrade<>'--'";
        ...
        public void UpdateTrainGradeByUserID(string userIDs, string Grade)
        {
            using (IDbConnection conn = OpenConnection())
            {
                string sql = string.Format(@"UPDATE Sys_User
set TrainGrade='{1}'
WHERE UserId in ({0})", userIDs, Grade);
                conn.Execute(sql);
            }
        }
        #endregion
        ...
        public List<Sys_Department> GetMyDepartMents(string jobNum)
        {
            using (var conn = OpenConnection())
            {
                var sql = "select * from Sys_Department where DepartmentId in (select distinct(deptid) from sys_user where leaderid='"+jobNum+"')";
