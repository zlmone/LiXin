...
select top {0} * from (
    select  row_number() over( {1} ) as rowNum
,dbo.F_NewIsAppStatus(Co_Course.Id,Cl_CourseOrder.UserId,3) as GetScoreFlag
...
select top {0} * from (
    select  row_number() over( {1} ) as rowNum
,case when (dbo.F_NewIsAppStatus(Cl_CourseOrder.CourseId,Cl_CourseOrder.UserId,Cl_CourseOrder.OrderStatus)=1) then Cl_CourseOrder.GetScore else 0 end GetScore
,Cl_CourseOrder.OrderStatus
,Cl_CourseOrder.MakeUpApprovalFlag
,Co_Course.CourseName as course_Name
,Co_Course.CourseLength
...
where rowNum between  @pageCount*@pageIndex+1 and @pageCount*(@pageIndex+1)", pageLength, orderBy);
        ...
        /// <param name="orderBy"></param>
        /// <returns></returns>
        public List<LiXinModels.CourseManage.Co_Course> GetMyCourseCPAList(int userId,
                                                                           ...
                                                                           string orderBy = " id DESC")
        {
            using (IDbConnection connection = OpenConnection())
            {
                ...
                string query = string.Format(@" select * from (
                    select  row_number() over( order by {0}) as rowNum,count(*)OVER(PARTITION BY NULL) totalcount,* FROM (
                    SELECT  isnull(Cl_CpaLearnStatus.id,0) as MyStatus
                        ,Co_Course.* , Cl_CpaLearnStatus.UserID cluserID,
                        (select count(0) from Cl_CpaLearnStatus 
                            left join Sys_user on  Cl_CpaLearnStatus.UserId = Sys_user.userid 
                            where Cl_CpaLearnStatus.courseid = Co_Course.id 
                            and Sys_user.IsDelete = 0
                            --and  Sys_user.Traingrade in (select id from dbo.F_SplitIDs(Co_Course.OpenLevel))
                    ...
                    from	Co_Course 
                    left join (select * from Cl_CpaLearnStatus where Cl_CpaLearnStatus.userid = {1} )  Cl_CpaLearnStatus 
                            on Co_Course.id=Cl_CpaLearnStatus.courseid 
                            )a
                    where {2}
  
                ) result 
                where rowNum between  @pageCount*(@pageIndex-1)+1 and @pageCount*@pageIndex", orderBy,
                                             userId, where);
...
(
	...
	WHERE " + where + @" 
	and    Cl_CourseOrder.UserId={0} and Cl_CourseOrder.OrderStatus in(1,2,3) and Co_Course.IsDelete=0
) bb
", userId)).First();
...
(
	SELECT 
		...
		,Co_Course.EndTime
        ,Co_Course.CourseTimes   
        ,Co_Course.DepCourseId
        ,Cl_CourseOrder.IsAppoint    
		,Sys_User.Realname as TeacherName
		,Sys_ClassRoom.RoomName as RoomName
		,Cl_CourseOrder.PassStatus as PassStatus
		,CASE WHEN (dbo.F_NewIsAppStatus(Co_Course.Id,Cl_CourseOrder.UserId,Cl_CourseOrder.OrderStatus)=1) THEN Cl_CourseOrder.GetScore ELSE 0 END  as GetScore
		,Cl_CourseOrder.CourseId as CourseId
        ,Cl_CourseOrder.OrderTime
        ,Cl_CourseOrder.OrderStatus
        ,Cl_CourseOrder.MakeUpApprovalFlag
...
left join co_course on co_course.Id = {2} --sys_user.TrainGrade in (select id from dbo.F_SplitIDs(Co_Course.OpenLevel))
                                                                              ...
                                                                              "ORDER BY su.userId DESC")
        {
            ...
            {
                totalCount =
                    connection.Query<int>(string.Format(@"select count(1) from Cl_CpaLearnStatus,Sys_User su where su.UserId= Cl_CpaLearnStatus.UserID and Cl_CpaLearnStatus.CourseID = {0} and {1}", courseId, where)).First();
                var query = string.Format(@"
                select top {0} * from (
                    select  row_number() over( {1} ) as rowNum,su.Realname,su.UserId,su.TrainGrade,su.DeptName,su.DeptPath,su.Sex,Cl_CpaLearnStatus.LastUpdateTime as OrderTime
                    from Cl_CpaLearnStatus,Sys_User su
                    where su.UserId= Cl_CpaLearnStatus.UserID and su.IsDelete = 0 AND Cl_CpaLearnStatus.CourseID = {2} and " + where + @" 
                ) result 
            ...
            using (IDbConnection connection = OpenConnection())
            {
                ...
                totalCount =
                    connection.Query<int>("select count(1) from Co_Course where Teacher=" + teacher + " " + where +
                                          " and CourseFrom=2 and way=1 and Publishflag=1 and isdelete=0").First();
                ...
                sql += "  from Co_Course where way=1 and Publishflag=1 and isdelete=0 and Teacher=" + teacher + " " + where;
...
select 
 ...
 left join Sys_User on Sys_User.UserId=Cl_CourseOrder.UserId
  where ((Cl_CourseOrder.orderstatus = 1 and Cl_CourseOrder.isleave = 0 )
    ...
    or (Cl_CourseOrder.orderstatus = 1 and Cl_CourseOrder.isleave = 1 and Cl_CourseOrder.approvalflag = 1 
        and Cl_CourseOrder.approvaldatetime > Cl_CourseOrder.ApprovalLimitTime) 
    or Cl_CourseOrder.orderstatus = 3) and Cl_CourseOrder.CourseId={0} {1} and Co_Course.IsTest=1 and Co_Course.IsDelete=0
...
", courseId);
        ...
        /// 获取集中授课中报名的人员
        /// </summary>
        public List<Sys_User> GetCourseOrderName(int courseID, string where = "1=1", int pageSize = 10, int pageIndex = 1, string jsRenderSortField = " su.UserId asc")
        {
            using (var connection = OpenConnection())
            {
...
WHERE number BETWEEN @startLength*(@startIndex-1)+1  AND @startLength*@startIndex", courseID, where, jsRenderSortField);
  ...
  Co_Course.way,
  Cl_CourseOrder.GetScore as GetLength,
  Cl_CpaLearnStatus.CpaFlag as CpaLearnStatus_CpaFlag,
  Cl_CpaLearnStatus.GetLength as CpaLearnStatus_GetLength,
  Co_Course.StartTime as CourseStartTime,
  Co_Course.EndTime as CourseEndTime,
  Cl_CourseOrder.OrderStatus
   from Cl_CourseOrder left join Co_Course on Cl_CourseOrder.courseid=Co_Course.Id
					   left join Cl_CpaLearnStatus on Cl_CourseOrder.courseid=Cl_CpaLearnStatus.courseid  
   where Cl_CourseOrder.UserId={0} and Cl_CpaLearnStatus.CpaFlag in(1,2)
   and Cl_CourseOrder.OrderStatus in(1,3)", userid);

                return connection.Query<Cl_CourseOrder>(sql).ToList();
            }
        }
    ...
    from dep_course
	  left join dep_CourseOrder on dep_CourseOrder.courseid=dep_Course.Id
    where way=1 and dep_CourseOrder.UserId={0}", userid);
                return connection.Query<Cl_CourseOrder>(sql).ToList();
            }
  ...
  select count(1) a from  Cl_CpaLearnStatus
	left join Co_Course on Cl_CpaLearnStatus.CourseID=Co_Course.id 
   where CpaFlag=1 and Co_Course.way=3 and ismust=1 and userid={0} and YearPlan in ({1})", userid, year);
        ...
        public List<Cl_CourseOrder> GetCompletionDetial(string where = " 1=1",string timewhere=" 1=1")
        {
            using (var connection = OpenConnection())
            {
                var sql = string.Format(@"WITH viewCPA AS
(
SELECT yearplan,userid,sum(tscore) as tscore,
			sum(dScore) as dScore,
			sum(cpaScore) as cpaScore,
            sum(ismust) as ismust FROM view_CPAScore
WHERE {1}
GROUP BY yearplan,userid
)

select  tscore, dScore, cpaScore, ismust,
			sys_user.realname as realname,
			sys_user.TrainGrade as TrainGrade,
			sys_user.DeptPath as DeptName,
            sys_user.deptid as deptid,
            yearplan,
            sys_user.cpa as iscpa,
            sys_user.userid,Username          
			 from  sys_user
		left join viewCPA on sys_user.userid=viewCPA.userid
 where  sys_user.IsDelete=0 AND sys_user.Status=0 AND IsTeacher<2 AND UserType IN ('在职','见习','特批','聘用')
 and {0}", where, timewhere);
        ...
        public List<Cl_CourseOrder> GetAttendByCompletionDetial()
        {
            ...
            {
                var sql = string.Format(@"select * from (
select userid,0 as AttendStatus,Cl_Attendce.StartTime as AttendceStartTime,Cl_Attendce.EndTime as AttendceEndTime,co_course.AttFlag as AttFlag,co_course.StartTime as StartTime,co_course.EndTime as EndTime,1 course_type,-1 ApprovalFlag from dbo.Cl_Attendce 
		left join co_course on Cl_Attendce.CourseId=co_course.id
union
select userid,Dep_Attendce.Status as AttendStatus,Dep_Attendce.StartTime as AttendceStartTime,Dep_Attendce.EndTime as AttendceEndTime,dep_course.AttFlag as AttFlag,dep_course.StartTime as StartTime,dep_course.EndTime as EndTime,2 course_type,ApprovalFlag
 from dbo.Dep_Attendce 
	left join dep_course on dep_Attendce.CourseId=dep_course.id
UNION 
   select userid,DepTran_Attendce.Status as AttendStatus,DepTran_Attendce.StartTime as AttendceStartTime,DepTran_Attendce.EndTime as AttendceEndTime,Co_Course.AttFlag as AttFlag,Co_Course.StartTime as StartTime,Co_Course.EndTime as EndTime,2 course_type,ApprovalFlag
 from dbo.DepTran_Attendce 
	left join Co_Course on DepTran_Attendce.CourseId=Co_Course.id
)aa where  userid in (select userid from View_CheckUser) ");

                return connection.Query<Cl_CourseOrder>(sql).ToList();
            }
        }

        public List<Cl_CourseOrder> GetDepartment(string where = " 1=1")
        {
            using (var connection = OpenConnection())
            {
                var sql = string.Format(@"
SELECT * FROM(
	 	 select DISTINCT sys_user.deptid as deptid,
	 SUBSTRING(sys_user.deptpath,patindex('%/%',sys_user.deptpath)+1,30) as deptname
    from  sys_user
    where   DeptId>0 AND {0} 
) t  ORDER BY CASE WHEN patindex('上海%',deptname)>0 
          THEN '1' WHEN patindex('评估上海总部%',deptname)>0 
          then '2' ELSE deptname END,deptid
	 ", where);
                return connection.Query<Cl_CourseOrder>(sql).ToList();
            }
        }
        ...
        /// <param name="mySubType">预订状态</param>
        /// <param name="where">条件语句</param>
        /// <param name="startIndex">页所引</param>
        /// <param name="pageLength">每页大小</param>
        /// <param name="orderBy">排序</param>
        /// <returns></returns>
        public List<Co_Course> GetMyBroadcastList(out int totalCount, string mySubType, string where = "",
...
(
	...
	left join Sys_ClassRoom scr on  cc.RoomId=scr.id --关联教室
    left join (select id,code, row_number() OVER(PARTITION BY cc1.Code ORDER BY cc1.StartTime asc) as CourseTimesOrder,
                       ...
                       from Co_Course cc1 where cc1.isdelete = 0 and cc1.coursefrom = 2 and cc1.publishflag = 1) tt 
