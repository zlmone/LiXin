        ...
        static void FileTask(object fileModel)
        {
            ...
            try
            {
                ...
                switch (suffix)
                {
                    ...
                    case ".rmvb":
                    case ".wmv":
                        Media2Flv.MediaConvert(file.SourcePath, file.TargetPath);
                        break;
                    case ".flv":
                        var isFlv = System.Configuration.ConfigurationManager.AppSettings["IsFlv"];
                        if (string.IsNullOrWhiteSpace(isFlv))
                            break;
                        if (isFlv == "1")
                            Media2Flv.MediaConvert(file.SourcePath, file.TargetPath);
                        break;
                    ...
                    case ".xlsx":
                    case ".txt":
                        File2Pdf.File2PdfAspose(file, suffix);
                        break;
                    case ".pdf":
