...
namespace LiXinInterface.Examination
{
    public interface IExamination
    {
        ...
        /// <returns></returns>
        List<tbExamSendStudent> GetExamSendStudentByDeptSelfList(int depCourseId, int examPaperId);

        /// <summary>
        /// 找出集中 视频 部门自学所有考试数据
        /// </summary>
        /// <returns></returns>
        List<tbExamSendStudent> GetALLExamByOneTwoFive();

        List<tbExamSendStudent> GetPassOnlineNum(string year, int userid);

        List<tbExamSendStudent> GetPassOnlineNumForDep(string year, int userid);
